...
2. ✅ **I + J — FATTO 2026-09-11**: nona radice ratificata (TG msg 2154) → [[training-taxonomy/class-information-architecture]] con due figlie: [[training-taxonomy/class-information-presentation-structure]] (I+J) e [[training-taxonomy/class-module-boundary-flow-convergence]] (idea 12 di Fra, msg 2155; passo (b) del piano msg 2169). Contenuti **non revisionati**.
   **Stato al 2026-09-11 sera (piano a→b→c, msg 2169)**: (a) triage delle 11 idee **completo** — tre proposte scritte e in attesa di ratifica (registro D8: `class-confident-first-sequencing`, faccia ORDINE di `instruction-phase-clarification`, `class-recurring-signal-triage`); (b) **fatto**; (c) = il punto 4 qui sotto, e **con (c) si apre la discussione sul base model** (msg 2169).
...
4. **Fixture e scorer** delle classi proposte — finché non esistono, «classe scritta» significa *argomentata*, non *validata*, e la distinzione va detta ogni volta. **Stato 2026-09-11 sera**: il fixture-runner R8 è **completo** (`turns` + `pair` + modello vero via `eval/run-scene.mjs`); **4 classi hanno un lab eseguibile in coppia** (`self-sealing-decision`, `consumption-scale-for-budget`, `design-artifact-lifecycle`, `module-boundary-flow-convergence`), gate a 16 lab; primo modello gradato (F41: i due modelli locali 0/3 sul canary). Il contenuto delle classi resta **non revisionato** da Fra.
