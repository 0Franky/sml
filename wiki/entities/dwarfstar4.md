...
> **✅ Update 2026-07-08 (utente msg 1346/1353) — CLARIFICATION RISOLTA**: DS4 è il **motore per far girare il TEACHER**. L'utente conferma (msg 1353): *"DS4 come detto è solo per DeepSeek, quindi parlo di usare DeepSeek come teacher"* → **DS4 = engine di inferenza per DeepSeek-V4 usato come TEACHER nella distillazione** ([[../../project_teacher_deepseek_v4]]), **NON** un re-scope del serving del nostro SLM (che resta vLLM `--enable-lora` three-tier). Quindi la tensione "single-model DeepSeek-specifico" **non è un problema**: DS4 gira DeepSeek, che è esattamente ciò che ci serve dal teacher.
> **🆕 Inferenza DISTRIBUITA (utente msg 1353, "salvalo in memoria")**: DS4 supporta anche **inferenza distribuita** (multi-nodo) → **mitiga il blocco HW locale** (16GB RAM): il teacher DeepSeek-V4 può girare distribuito su più risorse invece che su una singola macchina da 96-128GB. Riclassifica il vincolo HW da "infattibile in locale" a "fattibile via distribuito/cloud".
> **🔮 Futuro possibile (msg 1353)**: se antirez vorrà dare supporto, potrebbe fare un **"modello differenziale"** (adattamento DS4) **anche per il NOSTRO LM** → DS4 diventerebbe engine anche per il nostro SLM, non solo per il teacher. `[SPECULATIVE — possibilità, non impegno]`.
