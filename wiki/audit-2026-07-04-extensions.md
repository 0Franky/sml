...
- **C1 [VERIFY] P1 — lockdown/INGRESS secret esfiltrabile via tool MCP strutturati.** `checkSink` per un secret SENZA `allowedSinks` calcola `reason` solo da fileExfil o host-in-URL; se nessuno dei due è presente → `allowed:true`. L'hook `tool_call` inietta i sealed ref in OGNI tool tranne `http_request`. Un `INGRESS_N`/`load_secrets_from_env` (che partono in lockdown, no allowedSinks) messo in un tool MCP che esce senza URL negli args (telegram reply, gmail create_draft, drive create_file) → valore reale iniettato e spedito. `src/sealed-secrets.mjs:504-519` + `secrets-guardrail.ts:401-437`. **Fix proposto**: per un secret in lockdown (no allowedSinks), default-DENY iniezione salvo shell locale verificabile; "nessun host identificabile" = fail-CLOSED per tool non-bash/strutturati.
...
- **C3 [TRACK] P1 — envelope untrusted `tool_result` aggirabile.** `src/tool-result-envelope.mjs`: (a) idempotenza decisa su contenuto attacker-controllato (`ALREADY_FRAMED = /^<tool_result\b/`); (b) testo non escapato → un `</tool_result>` a metà content rompe l'envelope. Injection annidata (la difesa P0 019f1d67 aggirabile). Fix: neutralizzare `<tool_result`/`</tool_result>` nel testo untrusted prima di wrappare + idempotenza non gated sul contenuto.
...
- **D3 [TRACK] P2(latent) — `session-context.mjs:11` `_convId` è un module-global**: 2 sessioni concorrenti nello stesso processo → l'ultima `session_start` vince → capture/lane/eviction leggono il convId sbagliato. Non scatta nel TUI 1-sessione-per-processo. Fix: threading convId per-call via ctx.
...
## Sintesi
- **Fixati in questo ciclo (7)**: C2, B1, B2, A1, D1, D2 + il P0 busy_timeout (già committato `ed453d2`).
- **Da verificare a mano (1, P1 sicurezza)**: C1 (lockdown-exfil) — richiede attenzione per non bloccare uso legittimo.
- **Tracciati (P1-P2)**: C3, C5, B3, B4, A2, A3, D3.
