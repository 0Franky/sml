...
### 5. Compaction "a MATRIOSKA" (nested-vision) per i casi estremi `[EXTRACTED — utente msg 424]`
...
3. **Profondità limitata**: la matrioska non deve annidarsi all'infinito (budget profondità ≤ ~2-3, come [[../training-taxonomy/area-01-organization-planning|hierarchical-decomposition]]).
...
- `request_compaction` ([[../wiki/todo|todo §NEXT BUILD item-0b]]) diventa **`request_focus`/nested-compact** (zoom-in matrioska) oltre che trigger; la compaction nativa di pi si disabilita.
- **cache-stable-prefix** (item-1, ✅ FATTO 2026-06-29: `absoluteTimestamps` + anchor `<current_time>` + tiebreaker, smoke 18/18) resta valido: il workspace di Strada 2 eredita il contratto stabile-in-testa / volatile-in-fondo.
