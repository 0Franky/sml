...
1. **Trigger**: quando passare da riordino-priorità → matrioska? Soglia su #item-in-watch/#task-aperti **o** su budget-token del workspace. → **Decisione utente (TG msg 464): ENTRAMBI i trigger validi** (si attiva al primo che scatta: #item-in-watch sopra soglia **OR** budget-token superato). La tua euristica "prima riordino, poi compact" resta (proporzionalità). Soglie numeriche da calibrare empiricamente nel build; analisi con agenti specializzati pianificata.
