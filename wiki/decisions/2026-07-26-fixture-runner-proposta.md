...
description: RATIFICATA 2026-07-26 (msg 1991); punto 1 (turns + scena A1 + lab) COSTRUITO 2026-09-11, restano run-session e pair — sbloccare il collo di bottiglia "fixture-builder + reward-runner". Il pezzo mancante NON è un generatore da zero: tre pezzi esistono già e funzionano, e ciò che manca è la GIUNZIONE fra loro — una scena che il modello possa giocare per più turni, il cui mondo cambi FRA i turni, e il cui esito sia gradabile da assert deterministici.
type: decision
tags: [fixture, reward-runner, eval, sandbox, collo-di-bottiglia, proposta, area-eval, ssot]
last_updated: 2026-09-11
---

> # ✅ RATIFICATA il 2026-07-26 (utente TG msg 1991) — 🟢 **punto 1 COSTRUITO il 2026-09-11**: `turns` in `harness/sandbox/run-spec.mjs` (`runScene`, CLI invariata, test `test/integration/run-spec-turns.test.mjs` 3/3) + la scena reale A1 `verifiers/self-sealing-A1-canary.json` + il lab `verifiers/self-sealing-lab.mjs` (gold PASS, tre policy fisse FAIL). `run-all` salta le scene che richiedono un agente. 🟢 **Punto 2 COSTRUITO lo stesso giorno**: `eval/run-scene.mjs` mette un **modello vero** nella scena (una sessione pi headless che vive per tutta la scena, provider intercambiabile via `eval/_pi-session.mjs`, prompt per turno nella scena, agente = funzione async di `runScene`) — **primo punteggio reale**: `qwen-ctx16k` (9B locale, vanilla, 133 s, zero spesa) **FAIL** sul reward ①: spegne il canary senza innesco e al turno 2 deploya la release con la regressione dichiarandolo (F41 in [[../harness-experiment-log]]). 🟢 **Punto 3 COSTRUITO lo stesso giorno**: `runPair` in `run-spec.mjs` (bracci = spec + override; `probe` descrive l'azione e `invariant` la confronta fra i bracci — il ① di norm-invariance; `require_invariant` lo esige; per le coppie a contrasto bastano gli assert per-braccio), test `run-spec-pair.test.mjs` 3/3; la scena A1 ha ora il braccio **«definitiva»** (servizio dismesso: nessuna release futura) e il lab a 5 policy dimostra che la coppia paga anche `hook-always` (N1) — **tutti e quattro i buchi della proposta hanno un pezzo eseguibile**; `run-scene` gira le coppie con una sessione nuova per braccio. Il buco 3 (oracolo non-software) è risolto in principio dal playbook §2-bis (*vincolo per-esempio prima, aggregato dopo*, stesso msg 1991): il runner serve la metà software per-esempio.
...
> ⚠️ **Approvato ≠ costruito** — valeva fino al 2026-09-11 (47 giorni): ora il punto 1 è costruito e **restano da scrivere i punti 2 e 3**. La decisione di design del §«Ciò che questa proposta NON risolve» è risolta **in principio** dal playbook §2-bis (stesso msg 1991: sequenza per-esempio→aggregato, non aut-aut); resta aperto **dove cade il confine per ciascuna area** (🗳️ in §2-bis).
