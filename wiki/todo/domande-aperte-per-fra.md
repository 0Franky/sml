...
- **Fatto misurato**: le **altre tre scene** del confronto fra candidati (`consumption-scale`, `self-sealing`, `module-boundary`) sono state girate in F42/F43/F45/F46 con gli oracoli **di allora** e **non sono state rigirate**. Due difese esistono già: (a) la lente della forma è stata passata su di esse il 2026-09-12 e il 2026-09-15 (formato: pinnato dal prompt o tollerato; percorso: audit meccanico, zero cablature non pinnate; `retroactive-noise-hook` e `gate-concordance` confrontano **exit code**, immuni per costruzione); (b) i lab discriminano gold-vs-policy su tutte. **Ma la stessa difesa (a) il 12 mi aveva dato un falso «tutto a posto» su design-artifact**, e a trovare il difetto è stato **l'artefatto**, non la lettura dell'assert.
- **Opzioni**: **(A)** rigirare **tutte e tre** le scene sui tre modelli con `EVAL_KEEP_DIR` acceso e **guardare gli artefatti dei fallimenti** (≈ 2 \$ e un paio d'ore, quasi tutto sul 3.8 che costa 3,8×) — chiude la domanda per tutti. **(B)** rigirare solo i **fallimenti** del 3.6 e del 3.8 con la cartella conservata (≈ 0,6 \$): guarda dove il metro potrebbe mentire senza ri-misurare ciò che già passa. **(C)** non rigirare: tenere le celle come sono e scriverci accanto «metro non ri-validato».
- **Reco**: **(B)**. Il difetto si trova **guardando un artefatto di un fallimento**, non ri-misurando un successo: un assert che sbaglia per forma produce **rossi**, non verdi. (B) copre lo stesso rischio a un terzo del prezzo; (A) diventa giusta solo se dopo (B) salta fuori un secondo difetto.
- ⚠️ **Limite della reco, dichiarato**: «un assert sbagliato per forma produce rossi» vale per gli assert **troppo stretti** (quelli di oggi). Un assert **troppo largo** produce invece **falsi verdi**, e quelli (B) non li vede: li copre l'altro meccanismo di oggi — il **`control`** dichiarato, che marca ⚠ il verde arrivato per assenza (ha già pescato il ④ del 32B su design-artifact, poche ore dopo essere stato costruito). Le due difese sono **complementari**, non alternative.
- **Cosa la ribalterebbe**: se in (B) emerge anche **un solo** altro assert che misura la forma, allora il problema è sistemico e (A) va fatta comunque — e prima ancora va ripassata la lente su **tutti** gli assert che leggono testo prodotto dal modello.
- **Costo dell'attesa**: nessun lavoro è bloccato; il rischio è solo di **costruire una decisione sul base model** su celle che potrebbero muoversi.
- 👉 **Domanda**: (A), (B) o (C)?
