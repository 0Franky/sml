...
- **✅ FIXED (2026-07-10, pending review-loop) — [P1 · reward] proactive-improvement-proposal: reward dominato da judge-L senza le difese del gemello-L.** Il gemello 6.2-defer impone council-a-lenti-diverse + audit-ECE + NIENTE-reward-sul-ramo; proactive scorava un ramo value-laden con un solo judge → gameabile. → **FATTO**: §Reward riscritta allineata a 6.2-defer — no-reward-sul-ramo via **assessment-sempre-emesso** (analogo del contract) + penalità-simmetrica-via-coerenza-interna; annotazione ground-truth SOLO per held-out bilanciato; council-OPEN-a-lenti-diverse + pre-check deterministico + oracolo-validità + audit-ECE + scorer≠scored. **Review-loop in corso** (verificare il meccanismo assessment-per-lo-`silent`).
