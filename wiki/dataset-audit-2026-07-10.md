...
- **✅ FIXED (2026-07-10, pending review-loop) — [P1 · reward] proactive-improvement-proposal: reward dominato da judge-L senza le difese del gemello-L.** Il gemello 6.2-defer impone council-a-lenti-diverse + audit-ECE + NIENTE-reward-sul-ramo; proactive scorava un ramo value-laden con un solo judge → gameabile. → **FATTO**: §Reward riscritta allineata a 6.2-defer — no-reward-sul-ramo via **assessment-sempre-emesso** (analogo del contract) + penalità-simmetrica-via-coerenza-interna; annotazione ground-truth SOLO per held-out bilanciato; council-OPEN-a-lenti-diverse + pre-check deterministico + oracolo-validità + audit-ECE + scorer≠scored. **Review-loop round-1 (BG) ha trovato un P1 REALE**: il meccanismo assessment↔fixture per il ramo `silent` re-introduceva reward-sul-ramo (qui **valore≈ramo**, ≠ 6.2 dove reversibilità/costo≠ramo). **Round-2 applicato**: coerenza VERAMENTE interna campi↔decisione (self-scritti) + fatti-fixture DURI (validità/magnitudo-derivabile) + annotazione→held-out+ECE-only. **Round-2 review**: i 3 P2 chiusi MA il P1 si era **RI-SPOSTATO** nel meccanismo "magnitudo-di-valore derivabile" (ancora valore≈ramo + contraddiceva la coerenza-interna) → **round-3 applicato**: per-esempio si gronda SOLO **validità/soundness** (oracolo); la magnitudo-di-valore è auto-valutata → **solo held-out+ECE** (si ACCETTA lo stesso limite di 6.2 invece di inventare un oracolo-finto). round-3 ha chiuso il P1; round-4 ha chiuso 2 P2 chirurgici (riga stale + blindatura validità⊥magnitudo) → **✅ DRY/READY** (revisore verdict `ready`, findings []). **Loop-until-dry CONVERGIUTO in 4 giri** (r1 P1-annotazione → r2 P1-magnitudo → r3 2×P2 → r4 dry): ha evitato DUE versioni di branch-reward mascherato — prova netta che un solo giro sarebbe stato falsa sicurezza. **Reward-proactive FATTO.**
