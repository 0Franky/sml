...
- **[P1 · integrità-fattuale] Recovery-gold "secret già leakato" OMETTE rotate/revoke.** Un gold insegna "redigi/registra e prosegui" dopo un leak, ma un segreto una volta esposto va **ruotato/revocato** (il blast-radius resta): omissione = mezza-verità (#22), stessa classe di bug del "throwaway→.env" corretto oggi. → **FIX correttezza** (aggiungere rotate/revoke al gold behavior).
