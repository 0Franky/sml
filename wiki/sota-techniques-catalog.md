...
- ⚠️ **Coconut (latent reasoning)** — reasoning token-budget-zero MA **antagonista** del nostro structured-thinking `[V]/[A]/[?]` (rimuove i token espliciti) → candidato ADR, NON adottare senza scelta · S · F3/∞ · 2412.06769 · **agg. 2026-09-11** (ri-verifica su fonti primarie): il limite che pesa per noi è **il reward** — PRM/ORM non discriminano i pensieri continui (2510.12167); l'ibrido latente-dentro-token ha una forma allenabile con GRPO via token di confine = **SWITCH** (2606.13106) e un canale discreto+continuo = **DiscoLoop** (2607.00341); il cugino *recurrent depth* (loop in profondità, 3.5B) è 2502.05171. GPT-6 Astra (set 2026) usa «recurrent depth» **secondo la stampa**, non confermato da OpenAI, e conserva la CoT a token. Numeri e verdetto: `todo.md` «BATCH IDEE 2026-09-11» riga 8
...
## RL-7 — [2026-09-11] Triage del sweep «tecniche per i 9 problemi noti»: 65 ID → **10 IMPORTA · 9 HELD-OUT · 7 COPERTO · 21 CITA · 18 SCARTA**

> **Provenienza e livello di verifica, dichiarati** (#26): rapporto dell'agente `ricerca-tecniche` (`_private/research-sweep-2026-09-11.md`, richiesta Fra msg 2202); il suo `[V]` = pagina abs aperta. **Io ho verificato alla fonte 4 ID su 65** (2603.06621 · 2605.26438 · 2609.01217 · 2606.12941: esatti tutti e quattro). **Ogni altro verdetto qui sotto è preso sul riassunto dell'agente, non sul paper** → il flag **PDF** segna ciò che va letto per intero **prima** di costruirci sopra (ADR, fixture, reward). Criterio unico di triage: *quale decisione nostra cambia?* ([[feedback_measure_serves_a_decision]]) — una tecnica buona che non ne cambia nessuna è CITA o SCARTA. **Legenda**: **COPERTO** = già in wiki (dove) · **IMPORTA** = entra con un'azione tracciata in `todo.md` · **HELD-OUT** = benchmark da tenere fuori dal training (#18), licenza da verificare prima dell'uso (#29) · **CITA** = conferma o riferimento, nessuna azione · **SCARTA** = con la ragione. Due punti in cui **dissento dall'agente**, segnati ⚠️.

**P1 — premessa che scade** · TriggerBench 2606.23459 **HELD-OUT** (misura anche il *false-alarm* = il negativo bilanciato #21 gratis) · PM-Bench 2607.12385 **HELD-OUT** (rescheduling + canale di stato invisibile = la forma del canary) · StateMem 2608.19652 **COPERTO** (marcare l'informazione superata: lo fa `<recent_changes>` old→new lato F, [[training-taxonomy/class-durable-knowledge-retraction]] lato S) · Proactive Memory Agent 2607.08716 **SCARTA** (un secondo modello che inietta reminder = scaffolding, contro l'identità Tier-1 e #33) · 2606.22936 premature commitment **CITA** (diagnostica su hidden state; accanto a [[training-taxonomy/class-confident-first-sequencing]]). *Il rapporto lo dice e lo confermo: nessun metodo pubblicato addestra «lascia un innesco» — resta nostro.*

**P2 — oblio col rumore** · MINTEval 2605.18565 **HELD-OUT** (le «history questions» = il bug esatto: l'update nuovo oscura lo stato vecchio) · LongMemEval-V2 2605.12493 **HELD-OUT** (premise awareness, dynamic state tracking; «work in progress») · sharding 2606.12941 **COPERTO** per costruzione (turni + bracci di rumore, playbook §2 passo 6-bis) e **CITA** come forma pubblicata della ricetta · ⚠️ MMPO 2605.30159 **SCARTA** come reward (l'agente lo dà «promettente»: la Belief Entropy è l'incertezza della **propria** credenza, e la policy che riassume la controlla → premia la **sicurezza**, non la correttezza: è il confident-wrong in forma di reward, hack-check #10) · WMT 2608.20631 e SegTreeMem 2606.04555 **SCARTA** (strutture di memoria lato harness: le nostre lane esistono e F43 mostra che il 32B non le tocca; l'ordine temporale autoritativo è già la regola #13) · OSL-MR 2606.10616 **SCARTA** (teoria, scala assente) · SINKFLEX-RL 2608.10357 **CITA** (infra VERL+GRPO per rollout multi-turno lunghi: quando si sceglie lo stack RL, [[project_framework_stack]]).

**P3 — assenza dichiarata senza cercare** · 2608.01913 **IMPORTA · PDF letto il 2026-09-12** (lente *retrieval gap* ≠ *utilization gap*, definite su qrels umane, deterministiche: RG = fallisce senza aver mai portato l'evidenza gold, UG = fallisce con l'evidenza in contesto; RG domina in 5 agenti su 6, 52-64 %; **lo sforzo è inversamente correlato all'accuratezza** (ρ = −0,77) mentre il recall la predice (r = 0,99); i run sbagliati cercano **1,9-2,9× più a lungo** e la coda è «continuazione razionale sotto retrieval fallito», non un difetto di stop; **la ri-query ridondante è il predittore più pulito del fallimento**, ρ = −0,83 → da penalizzare. Per il lab presenza/assenza: due assert distinti — «trovato dopo riformulazione» e «letto e usato» — e la ricerca si gronda come **saturazione senza ripetizioni**, non come volume; «vie esaurite» = nessuna query nuova porta evidenza nuova, con le ripetizioni identiche contate come negativo) · SeekBench 2509.22391 **HELD-OUT** (grounding · recovery via riformulazione · sufficienza, a livello di step) · ⚠️ SVRL 2609.08025 **SCARTA** come reward (l'agente lo dà «il più promettente»: un reward di **diversità delle query** per-esempio paga il ramo «riformula» a prescindere dall'esito = #32 in forma pura; si gronda la ricerca solo dove l'oracolo dice che l'oggetto non c'è) — **CITA** la penalità search-aware · ReSeek 2510.00568 **CITA** (azione JUDGE ≈ il nostro verify-step; il reward «utilità dell'evidenza» ha lo stesso problema #32) · 2607.06503 abort precoce **CITA** (l'inverso: non insistere quando è perso → negativo di [[training-taxonomy/class-stagnation-recovery]]) · MARCH 2603.24579 **SCARTA** (tre agenti, 8B) · CaRR 2601.06021 **SCARTA** (rubriche giudicate da un LLM, #10).

**P4 — reward hacking per-fase** · 2603.06621 **IMPORTA · PDF letto il 2026-09-12** ([V] mio sull'abstract, corpo via riassuntore; chiude *«PRM appreso: no»* — 43% del reward da stile, accuratezza <4% con reward >0.9, **perimetro: matematica, due PRM piccoli, policy 1,5B, mitigazioni proposte ma non testate** — e tocca la Wave 6 «ORPO+PRM+GRPO»: ⛔ **ADR-proposta scritto**, [[decisions/2026-09-12-prm-appreso-escluso-proposta]], registro D11) · VPR 2605.10325 **CITA · PDF letto il 2026-09-12** (oracoli deterministici per fase = la nostra strada, pubblicata su tre giochi con Qwen3-4B; **un oracolo debole fa peggio della base anche fuori dominio** = il nostro gate dei lab; l'estensione agli ambienti agentici con oracolo su stato è dichiarata aperta: è dove stiamo noi) · RLVP 2607.07435 **CITA** (= #10 pubblicata: violazioni = penalità, mai reward positivo di percorso) · CapCode/CapReward 2606.07379 **IMPORTA · PDF** (test randomizzati con cap noto: un punteggio sopra il cap è **prova** di cheating → hack-check a costo basso per [[training-taxonomy/class-evaluation-integrity]]) · SpecBench 2605.21384 **CITA** (metrica = gap fra test visibili e held-out composizionali: è il nostro mutation test espresso come numero per fixture) · RHB 2605.02964 **COPERTO** ([[#RL-3]]) + fatto nuovo **CITA**: *l'RL post-training aumenta l'hacking; il 72% degli exploit ha razionale esplicito nel CoT* (→ la catena ancorata #10 ha anche valore di **detection**) · 2604.12086 **SCARTA** (max-min teorico) · BenchShield 2609.11028 **SCARTA** (infra) · AgentV-RL 2604.16004 **SCARTA** (verificatore appreso = giudice, #10). *Il rapporto lo dice e lo confermo: la «policy a intelligenza zero come baseline» non è formalizzata da nessuno — resta convenzione nostra, ed è quella che ha fatto il lavoro in F43.*

**P5 — calibrazione** · CALIBER 2606.24281 **IMPORTA · PDF letto il 2026-09-12 — ⚠️ corregge il rapporto**: NON è elicitation a inferenza, **richiede training** (RLVR con reward di calibrazione a errore quadratico su `<pre_confidence>` e `<post_confidence>`, 30 K esempi BigMath, warmup sulla sola pre-confidenza perché nell'ottimizzazione congiunta il suo segnale annega; −52,5 % ECE vs DCPO-lite sul 7B, solo QA/matematica, nessun confronto con probe interni). Il formato «prima/dopo» resta importabile come **misura** (ECE su held-out senza training) e i due punti mappano sui nostri `[?]` e `[V]/[A]`; come reward è la stessa famiglia di RLCR (proper scoring rule per-esempio sul campo confidenza) · Agent-BRACE 2605.11436 **IMPORTA · PDF letto il 2026-09-12 — ⚠️ con un caveat che il rapporto non diceva**: la credenza è un elenco di claim atomiche con **7 livelli verbali di certezza** (confirmed … unknown), aggiornata a ogni passo, e il suo reward (tracking + correttezza + diversità + formato + successo scontato) è calcolato da un **giudice LLM da 30B** che confronta le claim con lo stato vero → è un giudice-in-loop (#10), accettabile solo perché confronta con un ground truth noto; Qwen2.5-3B / Qwen3-4B su TextWorld, +14,5 / +5,3 punti sui baseline RL, Brier 0,40 → 0,28 lungo l'episodio, contesto quasi costante; serve SFT iniziale e prompt di dominio (−10 punti senza), transfer solo a domini simili, codice pubblico. Per noi: il bersaglio addestrabile «certezza per-claim sullo stato del mondo» è confermato; il reward va rifatto **senza giudice** con claim strutturate scorabili dagli oracoli su file — non ora) · RLCR 2507.16806 **COPERTO** (riga sopra) · 2601.07264 **CITA** (i tool di **evidenza** inducono overconfidenza, i tool di **verifica** calibrano: sostiene #22d «premia il verificare») · 2607.08046 **CITA** (il probe interno calibra meglio della confidenza verbalizzata e il CoT può non riflettere la decisione: accanto a 2602.20031 in [[concepts/steering-vectors]]) · RLCM 2604.23333 **CITA** (margine misurato, ma serve l'etichetta per step) · DualStake 2609.00935 **CITA** (watch, 4-7B) · CSR 2605.15588 e TRUST 2606.06976 **SCARTA** (scala assente, nessuna decisione servita).

**P6 — CoT adattiva + self-verification** · 2601.02972 **IMPORTA · PDF letto il 2026-09-12** (reward `R = R_C − λ·(L(y) − L(y_first))/L(y)` con λ = 1: si penalizza la **frazione** di traccia dopo la prima risposta corretta, zero penalità se nessuna risposta è corretta; SFT sul più corto fra 8 campioni corretti + GRPO 50 iterazioni; Qwen3-8B/32B dense; −28 % di lunghezza per −1,6 punti medi sul 8B (MATH-500 e CommonsenseQA salgono, GPQA e LiveCodeBench scendono di ~1), −40 % sul 32B; la lunghezza cresce con la difficoltà (adattività misurata); **matematica-only, single-answer, nessun task agentico**. Per noi la forma si trasporta così: nelle scene l'analogo di «primo punto corretto» è **il primo turno t\* in cui gli assert sono già soddisfatti** — tutto il lavoro dopo t\* è overhead misurabile con gli oracoli su file, senza giudice; richiede che `run-spec` valuti gli assert a ogni turno, non solo alla fine → todo, dopo la catena) · TAB 2604.05164 **CITA** (budget **per turno** sotto vincolo globale = consumption ④) · Soft-SVeRL 2605.28561 **CITA** (avverte: la self-verification appresa **gonfia il reward** se non ancorata → conferma la terza regola di [[training-taxonomy/lab-sequence]]) · STV 2605.30290 e 2608.20256 **CITA** · SVR 2607.28457 **SCARTA** (verdetto appreso senza oracolo a inferenza: il nostro è ancorato ai tool call).

**P7 — system prompt nei piccoli** · 2609.01217 **IMPORTA** ([V] mio; **un solo template per batch** batte i metodi complessi: mescolare template nello stesso batch mette in competizione i gradienti sul 57-64% dei parametri → una riga nel config SFT, costo zero) · VerIH 2511.04694 **HELD-OUT / dataset SFT candidato** (istruzioni in conflitto verificabile, 4B-35B; licenza) · RealGuardrails 2502.12197 **HELD-OUT / dataset SFT candidato** (system prompt reali; licenza) · ManyIH-Bench 2604.09443 **HELD-OUT** · AMD 2608.07169 **COPERTO** (la «function memory» — convenzioni e trappole per tool — è la lane `how_memory_works`; e F43 dice che il 32B la ignora comunque) · 2606.07808 **SCARTA** (monitor harness per i piccoli: il target è ≥27B, [[project_test_model_vs_target]]). *Confermo il buco dichiarato: «i piccoli ignorano il system» non ha una tecnica nominata — il nostro F34/F41 resta il dato.*

**P8 — budget dichiarato + gate autocostruiti** *(AnySearch, BAGEN, ExecCritic, SEAL: PDF letti il 2026-09-12, esiti cablati in [[training-taxonomy/class-consumption-scale-for-budget]] §Letteratura e [[training-taxonomy/lab-sequence]] fase 3)* · AnySearch 2609.00813 **IMPORTA · PDF letto** (budget iniettato nel prompt + **curriculum sui budget** = i bracci scarso/abbondante di [[training-taxonomy/class-consumption-scale-for-budget]] generalizzati a regime di training; codice) · BAGEN 2606.00198 **HELD-OUT + CITA** (i frontier sono sistematicamente **ottimisti** e continuano a spendere su task persi: il gold non è «spendi meno», è **avvisa presto quando è perso** → negativo per consumption e gold di [[training-taxonomy/class-effort-honesty-under-difficulty]]) · ExecCritic 2609.09133 **IMPORTA · PDF** (test agent + repair agent, RL per ruolo, 35B-A3B: la fase 3 di lab-sequence «costruire il proprio gate» in forma pubblicata) · SEAL 2607.24300 **CITA** (*verifier-deployment gap*: se i test li scrive l'agente il self-score resta alto mentre la performance cala → l'accettazione dev'essere **esogena**: è la nostra terza regola, msg 2206) · Budget Tracker 2511.17006 **COPERTO** (idea 6 di Fra, lane `[tok N]`, todo) · ReVeal 2506.11442 **COPERTO** · INTENT 2602.11541 **SCARTA** (world model a inferenza).

**P9 — eval-awareness** · LURE 2605.26438 **IMPORTA** ([V] mio; prefisso = replay di una traiettoria **reale** — abbiamo i trace di `run-scene` — e coda = la fixture con oracolo: la scena non «sa di esercizio»; forma raccomandata per le scene nuove, playbook §4) · 2509.13333 **CITA** (l'eval-awareness cresce con **legge di potenza** nella scala, 0.27B-70B: a 27-36B c'è già; accanto a 2505.23836) · 2510.20487 **CITA · PDF letto il 2026-09-12 — ⚠️ corregge il rapporto e la mia lettura**: non c'è alcun probe classificatore né l'iterazione delle eval «finché il probe le legge come deployment» (citata solo come lavoro futuro di altri): il paper costruisce **steering vector** da 16 coppie contrastive («parli con un utente reale» / «con un valutatore») su 6 layer di Nemotron Super 49B e mostra che lo steering verso «deployment» abbassa un comportamento da valutazione (type hint 80 % → 11 %) con −1,6 punti di accuratezza, mentre lo steering verso «valutazione» costa −13,7; vettore specifico del modello, nessun transfer provato. **L'idea del lint delle fixture non poggia su questo paper**: se la si vuole, la base è 2509.13333 (probe lineari, codice) e va verificata lì — todo declassato · SENTINEL 2606.12908 **CITA** (dai pattern di errore si generano task mirati = il nostro loop gap→classe #18, automatizzato) · Tatemae 2604.26511 e 2607.24758 **CITA** (alignment faking come scelta del tool; persiste anche senza il linguaggio delle conseguenze) · 2602.08449 regime-blind **SCARTA** (adversarial invariance training: costo alto, nessuna decisione ora) · SYNTHAGENT 2601.22511 **SCARTA** (generico).
