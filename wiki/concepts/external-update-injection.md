...
> ⚠️ **Aggiornamento di fattibilità `[VERIFICATO 2026-06-27 — ricerca + decisione utente msg 193]`**: la prima ipotesi (pause/resume mid-stream nativo) NON è il modo giusto, e **NON degradiamo a turn-boundary** (section≠turn, materialmente diversi — decisione utente: non compromettere un design deliberato per un limite del tool; pi è MIT+estendibile → lo implementiamo NOI). **Realizzazione corretta**: il section-boundary si fa lato wrapper/serving via **multi-call con `stop=["</section>"]`** (pattern MinD, arXiv 2505.19788) — ogni sezione è una chiamata che si ferma al tag `</section>`, il wrapper valuta/inietta l'`<update>`, poi riprende con una nuova chiamata che **riusa il pensiero via Automatic Prefix Caching** (ricomputo del solo update → costo basso, fattibile su vLLM stock). NON serve pause/resume nativo. ⚠️ **Valore gated sul TRAINING**: un modello non addestrato all'interruzione perde fino a −60% accuracy (arXiv 2510.11713: Reasoning-Leakage/Panic/Self-Doubt) → serve la skill [[interruption-robust-reasoning]]. **Caso d'uso primario (msg 203)**: un messaggio utente async iniettato al prossimo `</section>`. Le primitive "Streaming inference adapter: pausare/riprendere" più sotto vanno lette come *sostituite dal multi-call MinD*. Vedi [[../architecture/wrapper-implementation-plan]] §Step-0.0 div-1 e [[../architecture/harness-feature-catalog]] §Classe-B. Collocamento confermato **post-MVP / Fase 3** (per la dipendenza dal training, non per fattibilità).
