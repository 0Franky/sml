...
- **`open/stream_read_file` → `close_stream_file(file)`** — lo stream-read porta porzioni di file **inline** nel contesto (`<open_file_view>`); **`close_stream_file` lo cancella TOTALMENTE dal contesto**. È **context-eviction esplicita guidata dall'LLM** → istanza concreta dell'autocompact/context-edit ([[_user-notes-2026-06-23]] nota 5) e complemento di [[sliding-window-variable-tool]]. Token-saving: apri quando serve, chiudi quando hai finito.
  - ✅ **COSTRUITO 2026-07-16** (`harness/src/file-view.mjs` + `.pi/extensions/file-view.ts`, 37 test). Era rimasto backlog "Classe A" per ~3 settimane. `[EXTRACTED]` dal design; deviazioni **dichiarate**:
    - **Nome**: `open_file_view` / `close_file_view` (non `stream_read_file`/`close_stream_file`) — allineati alla lane `<open_file_view>` che il design stesso nomina, e onesti sul fatto che NON è uno stream (è una finestra ferma finché non la muovi). Rinominare era una scelta, non un refuso → segnalata all'utente (regola #34).
    - **NON c'è `list_file_views`**: la lane È già la lista, un tool-mirror pagherebbe uno schema per zero informazione ([[../harness-experiment-log|E14/F37/F39]]: gli schemi-tool dominano il costo).
    - **Cap `MAX_OPEN_VIEWS=3` che RIFIUTA** la 4ª apertura invece di sfrattare la più vecchia: lo sfratto silenzioso insegnerebbe che "le risorse si gestiscono da sole" — il rifiuto costringe la **decisione** (vincolo anti-proliferazione dell'utente, msg 1688: *"se ne crea 100 e tutti sparsi ne perde il controllo"*) → è il **segnale di training**, non burocrazia.
    - **Split #11**: il meccanismo (apri/tieni/chiudi) è **F-harness**; *quando* una porzione merita di restare / non serve più / conviene tenerla comunque è **S** → [[../training-taxonomy/class-memory-lane-tool-discipline]].
  - ⚠️ **Distinto da `sliding_var_read`** ([[sliding-window-variable-tool]], il *complemento*): quello legge una slice di una **var** e ritorna un **tool_result** → **scorre via col prune**. Qui è una **lane**, ri-emessa ogni turno → **sopravvive al prune** finché non chiudi. È la differenza tra *"read ridotta"* e *"tenere sott'occhio"* (chiarimento utente msg 1701: la seconda è ciò che aveva chiesto).
