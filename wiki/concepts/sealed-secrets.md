...
- **Semantica di `allowedSinks` — `[]` ≠ `["*"]`** (chiarito utente 2026-07-03): `[]` (vuoto) = **LOCKED** = NESSUNA destinazione permessa (in strict il secret non esce da nessuna parte finché non concedi un host con `request_sink`); `["*"]` = **QUALSIASI host** (massimo widening, NON concedibile in-sessione — solo out-of-band). Sono **OPPOSTI**: `[]`=zero, `[*]`=tutto. La lane `<secrets>` li rende **disambiguati** (`sinks=[] LOCKED` vs `sinks=[*]`) + footer esplicativo (`context-assembler.mjs`) — prima il rendering "none" era ambiguo e poteva far credere al modello che il secret fosse già usabile senza grant.
