...
- **Mitigazione (allow-shape, non deny-shape)**: il fast-path concede SOLO una shape pulita — `allowLocalHttp` ON · ≥1 host e TUTTI loopback-letterali · **un solo URL** (`countUrls` su `://`, anti `2://evil.com` invisibile a `extractHosts`) · niente scrittura-file · niente **host-pinning** (`--resolve`/`--connect-to`/proxy/`Host:` rimapperebbero "localhost" su un IP esterno) · niente **composizione di comandi** (`;`/`&&`/`|`/`$()`/`/dev/tcp`/newline = un secondo canale d'uscita, via `hasCommandComposition`). Qualsiasi violazione → si ricade su `https-only` (= blocco). `127.0.0.1.evil.com` (host che inizia con 127 ma risolve fuori) è respinto (match loopback solo su 4-ottetti).
