...
- **✅ PINNED (studio bound 8-agenti, 2026-06-30 → [[concepts/context-bounds-study]])**: la tensione prefisso-stabile è RISOLTA con precisione. La superficie self-tunable dal modello è **ESATTAMENTE 4 lane** (tutte volatile-tail): `<vars>` count · `<recent_changes>` count (mai la window) · `<messages_with_user>` n + charCap. **FROZEN-per-il-modello** (config write-time, NON P3): `task_list` (incl. il suo cap), `execution_order` (floor-ready frozen), tutta la testa `<frame>` — stanno nel prefisso/atemporale → resize = KV-thrash. **GATE proibiti** sulla superficie P3: `verify_queue`, `secrets`, `frame/constraints`. **3 BLOCKER prima di spedire** (P0/P1): (a) allocatore di **budget-token totale** (oggi NON esiste → "alza-tutto" è possibile); (b) i tool `set_view`/`expand_section` (oggi NON esistono, grep 0-hit); (c) il **lifecycle reset-to-default** (META + reset su pop_focus/nuova-sessione + decay K-turni). → P3 NON è costruibile finché questi 3 + la curva effective-context non atterrano.
...
8. **Lane `<progress>` / done-summary** (utente msg 740, 2026-06-30) — uno storico COMPRESSO dei passi compiuti. ⚠️ **NON va dentro le note** (= lesson-memo `[[error-memo-system]]`, ruolo errori/lezioni): riversarci un diario le inquina (`recall_lessons` → rumore) + è **duplicazione** del record F-harness (l'harness sa già quali task IN_PROGRESS→DONE + quali tool sono girati) + **vettore di reward-hacking** ("done X" scritto senza farlo = participation-hack, regola #11 + `feedback_reward_hacking_principle`). Forma sensata: un summary al confine di **compaction/checkpoint** (il tool `checkpoint` + handoff-note GIÀ esistono — `context-assembler.mjs:105` `buildResumeDigest`) o al massimo una lane `<progress>` dedicata e BOUNDED, separata dalle note. **Gap notes attuale** (risposta utente msg 740): il modello ha `remember_lesson` (create) + `recall_lessons` (recall) ma NON delete/edit espliciti (decay/archive è solo design di manutenzione). Il "diario dei passi" → progettarlo come `<progress>`/dentro current-step (P4), MAI nelle note.
