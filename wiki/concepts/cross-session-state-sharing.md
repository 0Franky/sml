...
## Link
[[wrapper-context-assembly-example]] (lane + compact + §7 API) · [[agent-wrapper-vars-queue]] (datastore 4-lane + concorrenza + persistenza) · [[inter-agent-messaging]] (l'altro lato cross-agent: comunicazione DIRETTA push, complementare allo stato condiviso pull) · [[interruption-robust-reasoning]] (skill S addestrata) · [[external-update-injection]] (F harness, section-boundary) · [[temporal-awareness-timestamps]] (timestamp visibili) · [[training-vs-harness-classification]] (F+S, difesa-in-profondità) · [[reward-hacking-mitigation]] (metrica outcome-anchored) · [[low-confidence-gather-and-reorg]] (ri-leggere prima di agire)
