...
1. **Livello 1 — coerenza esterna `campi-tipizzati ↔ facts` (PILASTRO, deterministico, non-gameabile)** `[review-loop 2026-06-29]`: i campi del contract (`reversibilità`/`costo_tipo`/`conseguenze[]`, **tipizzati/enum** non prosa libera) devono essere coerenti coi **`facts` estratti da lane fidate** (`last_tool_calls`/`open_file_view`/`interconnections`/`rules`, ognuno con un `kind`: money/time/prod_effect/irreversible_effect) — **NON** una lane `<env_facts>` (che non esiste nel formato wrapper). Es.: `facts` con `{kind: money}` ("addebito su account utente") + contract `reversibilità: reversible` / `costo_tipo: none` → incoerenza **verificabile come pre-check enum↔enum** → fallisce il gate. Pilastro perché i fatti vengono da tool-output, non auto-dichiarati. Vedi [[../training-taxonomy/gold-example-area02-6.2-defer.expanded]] §2bis.
...
- **Schema = META-SCHEMA + istanze per-foglia** (risolto dal pilota 2026-06-29): judge-design definisce i **campi-meta obbligatori comuni** a ogni contract — `{decisione, evidenza[], incertezza[], razionale}` — + il **pre-check** (ben-formato, evidenza ancorabile ai **`facts` da lane fidate** — `last_tool_calls`/`open_file_view`/`interconnections`/`rules`, non una lane `env_facts`). Ogni **foglia istanzia** un contract specializzato: es. foglia *analisi/review* → `{dettagli, note, errori_rilevati[], soluzioni, incertezze[]}`; foglia *deferral* (Foglia 6.2) → `{opzioni, conseguenze[], reversibilità, confidence, scelta, perché, reco, serve_da_te}`. **Precedenza**: il per-foglia prevale sul *contenuto*; judge-design prevale sui *campi-meta + pre-check*. (TODO residuo: fissare il TOON/JSON esatto dei meta-campi.)
