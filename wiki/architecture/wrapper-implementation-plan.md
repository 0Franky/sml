...
- **Context-assembly completo** (Classe A): lane aggiuntive *guidate dall'evidenza* (temporal, secrets-`SECRET#n`, history-2-livelli, block_notes, verify_queue, ~~open_file_view~~ ✅ **fatto 2026-07-16** — `src/file-view.mjs`, vedi [[../concepts/wrapper-context-assembly-example#7-harness-api--tool-lifecycle-design-utente-2026-06-23|§7]], messages_with_user) + **vars-queue** via `appendCustomEntry` + sliding-window var tool (`registerTool`) + **autocompact/`compact_context`** model-driven + degradation-awareness ([[../concepts/self-analysis-strategy-revision]]) + window enforced nell'hook `context`.
