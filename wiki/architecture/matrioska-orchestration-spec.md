...
- **Tool** (model-facing, F+S): `enter_focus({task_ids, reason?})` (guarded `canEnter` depth≤3) · `pop_focus({scope_id?})`.
  - **HARD-GATE focus-gathering v1 (2026-06-29)**: se nessun task del subset è **ready** (deps non soddisfatte) → `enter_focus` **rifiuta** `{reason:"no-ready-task", missing_deps:[...]}` (gemello del rifiuto subset-vuoto). `lead = primo task ready in execution-order` (mai un bloccato) — non più `subset[0]`. Vedi [[concepts/focus-task-prioritization]].
  - **`gathering.mode=require`** (opt-in): `enter_focus` bloccato finché il modello non ha consultato `get_execution_order` (token `_gather_token` consumato per-focus), gate su `minTasksForForce` open-tasks.
- **Hook**: `before_agent_start` assembla frame+focused-context+messages quando uno scope è aperto + inietta `<focus_hint>` quando pressione=matrioska e nessuno scope aperto (auto-suggest = floor S graceful; **`gathering.mode=inject`** allega la vista `<execution_order>` ordinata) + `<reorganize_hint>` quando pressione=reorder (anti-cecità, cooldown proprio) + **aim-in-coda** (`<current_aim_reminder>` ri-ancora l'aim in fondo) · `turn_end`/`context` chiama `getContextUsage()`+`evaluateTrigger` (con **output-reserve** opt-in) e persiste il livello in un `memo` · `session_before_compact` **defensive `{cancel:true}`** (compaction nativa OFF, la matrioska è il sostituto).
...
- **OQ-A (chi sceglie il sub-set del focus)** → **RISOLTA = model-initiated, con auto-enter opt-in** (config `autofocus.mode`, IMPLEMENTATO 2026-06-29 msg 551, commit `62bc936`). 3 livelli: **`off`** (nessun segnale), **`nudge`** (DEFAULT, invariato: `<focus_hint>` harness quando pressione=matrioska + la DECISIONE resta del modello, ADR principio-2 "il modello cura il suo workspace"), **`auto`** (l'harness ENTRA in focus DA SOLO sui task ready — top `focusK` in execution-order — quando pressione=matrioska, deterministico, con cooldown anti-thrash + rispetto depth/hard-gate; `maybeAutoFocus` node-pure). `auto` = rete di sicurezza per un modello che non si auto-organizza (es. SLM 4B); il dogfood Sonnet ha confermato che un modello capace si auto-organizza (→ `nudge` di default). Vedi [[concepts/focus-task-prioritization]].
