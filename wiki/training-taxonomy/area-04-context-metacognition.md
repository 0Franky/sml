...
- **Esempi**:
  - **(1) WITH-hint** — TUTTI gli hint, forte→debole:
    - *Hint forte (policy TTL)*: *"Ogni dato ha un TTL **sull'asse-tempo** (prezzi-mercato: 1min; tabella-di-conversione: ∞). Prima di usare un dato: `età = now - last_modified`; se `età > TTL` → RE-FETCH obbligatorio prima dell'uso. Non usare mai un dato stale."* → poi context con VAR datate.
      > 🔴 **CORREZIONE 2026-07-16** (utente msg 1717 + [[gap-report-2026-07-16]] gap A1): questo hint diceva **`codebase_tree: 1h`** e **`config-statica: ∞`**. **Sono FALSI** e insegnavano attivamente la cosa sbagliata: un albero del codebase e una config **non hanno un TTL** — sono risorse **condivise**, invalidabili **in qualsiasi istante** da un writer estraneo (l'utente nel suo IDE, un altro agente, la CI). *"Fresco di 5 secondi"* **non implica** *"valido"*. Sono stati **rimossi** dagli esempi: il TTL è la policy dell'**asse-A (churn proprio)** — quanto una cosa cambia **da sola** — e gli esempi devono restare su risorse **esclusive/non-contese** (i prezzi ticchettano da soli; una tabella di conversione non cambia).
      > **L'asse-B (chi altro ci scrive) NON è coperto dal TTL** e vive in [[class-concurrent-world-awareness]]: lì la validità si chiude su un **EVENTO**, non allo scadere di un timer, e nessun valore di TTL può catturarlo. **Le due policy compongono** e vanno insegnate insieme: l'età dice *quanto tempo ha avuto per cambiare*, l'altra dice *se esiste qualcuno che possa averlo cambiato*.
...
- **Esempi**:
  ...
  - **(4) WRONG — recovery** — come (3): il modello procede, poi un check a valle fallisce (comportamento incoerente) → **diagnostica** (c'era una contraddizione non risolta) → **la esplicita** (`<contradiction>`) → **risolve** con il repertorio sotto. Insegna il loop rileva→esplicita→risolvi.
    > 🆕 **Repertorio di risoluzione ESTESO 2026-07-16** (utente msg 1717: *"deve riuscire a **recepirle entrambe, unirle e proseguire su quella base**"* — [[gap-report-2026-07-16]] gap A1 faccia-iii). Prima erano solo **due** opzioni e **mancava la prima**, che è spesso quella giusta:
    > 1. **MERGE — recepirle ENTRAMBE** ⭐ *(default quando le due modifiche sono **ortogonali**: toccano campi/sezioni/aspetti diversi)*. Due cambiamenti concorrenti **non sono** necessariamente una contraddizione: se uno ha aggiunto `latte` e l'altro `pane`, l'esito corretto **contiene entrambi**. Scegliere-il-più-recente qui **CANCELLA** il lavoro altrui — è un clobber, ed è **irreversibile**.
    > 2. **SCEGLI con criterio** *(solo se genuinamente **incompatibili**: `mode=strict` vs `mode=lax` — non esiste una fusione valida)* — es. il più recente per timestamp (ponte Temporal/TTL), o l'autorità più alta.
    > 3. **CHIEDI all'utente** con contesto *(incompatibili + posta alta + nessun criterio dirimente)* — ponte Area 9 ask-vs-proceed.
    >
    > ⚠️ **Simmetria obbligatoria**: *"mergia sempre"* è un hack quanto *"last-write-wins"* → il merge di modifiche **incompatibili** produce un artefatto **invalido** (`mode=strict-lax` non parsa) → oracolo duro, non giudizio. Il discriminante **ortogonali-vs-incompatibili** è la skill. Vedi [[class-concurrent-world-awareness]] §(iii) + §N6.
  - **(5) OTHER — adversarial/edge**:
