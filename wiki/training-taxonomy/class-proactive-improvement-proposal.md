...
> ⚠️ **Il reward NON premia il RAMO (propose vs stay-silent)** — premiarlo degenera in "proponi-sempre"/"taci-sempre", il motivo per cui 6.2-defer non premia act-vs-defer (CLAUDE.md #10). *(Fix audit P1 + **review-loop 2026-07-10 round-1→4, DRY/READY** [loop-until-dry]: r1 groundava il value-field contro l'annotazione; r2 lo spostava su "magnitudo-di-valore derivabile" → **entrambi re-introducevano il branch-reward** perché **valore ≈ ramo** (soglia riga 16); r3 gronda per-esempio SOLO la **validità/soundness** (oracolo), la **magnitudo-di-valore** è auto-valutata e va **solo** su held-out+ECE — si ACCETTA lo stesso limite di 6.2 invece di un oracolo-finto; r4 chiude 2 P2 chirurgici → revisore verdict `ready`, findings [].)*
...
> **Assi ORTOGONALI (blindatura validità⊥magnitudo)**: *validità/soundness* = **CORRETTEZZA** del meccanismo (rompe / produce risultato sbagliato / lascia dato stale), **indipendente dalla magnitudo**. Un'ottimizzazione che **FUNZIONA** ma non conviene *a questa scala* (overhead > beneficio — es. parallelizzare un loop di 3 elementi) è **BASSO/NEGATIVO-VALORE** (→ solo held-out), **NON invalida** (→ mai groundata per-esempio). Così il fixture-author non può smugglare la magnitudo nell'oracolo-validità (chiude la "terza strada" del review-loop).
...
**Ground-truth di proponibilità → SOLO held-out + ECE, MAI label per-esempio**: l'annotazione "genuinamente-proponibile vs da-tacere" **costruisce** l'held-out bilanciato e **calibra** il giudice (ECE); **non** entra nel giudizio del singolo esempio (era la contraddizione flaggata dal review-loop). La **stabilità inter-annotatore** vale SOLO per la **validità by-construction** (fatto DURO di correttezza); **NON** per la magnitudo-di-valore (è il passo soft ≈-ramo → held-out+ECE, mai groundata per-esempio) né per un giudizio-di-valore soffice (integrità #22).
