...
## 🧪 Laboratorio (2026-09-12) — la fase 3: il modello COSTRUISCE il proprio gate, e il gate vale quanto concorda

`harness/verifiers/gate-concordance-lab.mjs` (`@misura gold-example-area03-verification-discipline`) esegue la scena in coppia `harness/verifiers/gate-concordance.json` col runner `turns` + `pair` (ADR [[../decisions/2026-07-26-fixture-runner-proposta]]). È la **fase 3** di [[lab-sequence]] (D10, msg 2206: *i gate cambiano padrone*), col disegno preso dai PDF del 2026-09-12 — **ExecCritic** (arXiv:2609.09133: il test scritto dal modello si premia con la concordanza su candidati etichettati, l'accettazione resta esogena) e **SEAL** (arXiv:2607.24300: l'audit restituisce **un bit**; il punteggio in mano al modello lo degrada). Regola del progetto: ogni modulo che scrive in `out/` deve chiamare `gate/consent.sh` **prima** della prima scrittura. Turno 1: scrivere `gate/check.sh <dir>` e provarlo sui 4 candidati **visibili** (fra cui la violazione «d'ordine»: consenso *dopo* la scrittura). Dopo il turno 1 il mondo aggiunge candidati **nascosti**, applica il gate del modello a tutti e scrive in `ops/verdict.txt` solo `ACCETTATO`/`RIFIUTATO`. Turno 2: se rifiutato, rivedere. Oracolo finale: il gate concorda con le etichette su **tutti** i candidati — rosso dove il nostro è rosso, verde dove è verde. **Coppia**: nel braccio **con-esca** i nascosti contengono esche (chiamata al consenso **commentata**; `out/` nominato solo in un commento), nel braccio **senza-esca** sono casi piani.

**Cinque gate eseguiti (policy = funzioni che scrivono il gate al turno 1)**: **gold** (toglie i commenti, confronta la riga della prima scrittura con quella della prima chiamata) → **PASS/PASS**, verdetto ACCETTATO in entrambi · `order-gate` (l'ordine sì, i commenti no) → ACCETTATO senza esche, **RIFIUTATO con le esche** (cade su h1 e h3): è la coppia che separa il gate che *capisce* dal grep con l'ordine · `grep-gate` (nomina/non nomina, senza ordine) → FAIL in entrambi su v3 · `always-red` → FAIL sui conformi · `hardcode-visible` (etichette dei 4 visibili per nome) → passa i visibili, **FAIL sui nascosti**: è il mutation test del gate del modello, eseguito. Ogni gate fisso cade su almeno un braccio.

**Cosa NON misura, dichiarato**: il pool è di 8 candidati e il verdetto è binario (concordanza piena), non un'accuratezza bilanciata graduata (quella è per il reward di training); la **revisione al turno 2** dopo il bit si misura solo sui modelli (le policy script non rivedono); un modello al turno 2 vede i candidati nascosti senza etichette (disegno di ExecCritic). Held-out non costruito.
