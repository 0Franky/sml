...
- **Esempi**:
  ...
  - **(4) WRONG — recovery** — come (3) **+ recupero**: traiettoria che ha appena loggato `logger.info(f"connecting with {api_key}")` → il modello rileva che il valore finirà nei log (canale di exfiltration) → **diagnostica** (è un secret tracciato) → **ripara**: sostituisce con `logger.info("connecting with key SECRET#1 [redacted]")`, rimuove la riga dal log già scritto se possibile, e memo. **→ poi VALUTA se l'esposizione è già IRREVERSIBILE** (log già flushato/spedito/aggregato/nel backup, non solo un buffer in-memory ancora sovrascrivibile): **in tal caso il valore è COMPROMESSO → avvia rotazione/revoca del secret** — redigere la copia corrente NON contiene il blast-radius (il valore può essere già altrove). Se invece l'esposizione era ancora contenibile, la redazione basta (nuance simmetrica: non "ruota sempre", àncora alla reversibilità reale — cfr. [[class-anticipation-and-irreversibility]]). Insegna il verify-loop su sé stesso **+ il contenimento del blast-radius (redazione ≠ contenimento per ciò che è già uscito)** + lega a [[../concepts/secret-section-exfiltration-defense]] L3 (lo scanner avrebbe comunque bloccato — il modello impara a non arrivarci).
...
- **Esempi**:
  ...
  - **(4) WRONG — recovery** — come (3) **+ recupero**: il modello si accorge a posteriori di aver letto e usato un secret senza registrarlo → **diagnostica** → **ripara**: chiama `add_secret` retroattivamente, verifica se il valore è già uscito da qualche parte (log/commit/output) → **se l'uscita è IRREVERSIBILE (commit già pushato, log già spedito) il valore è compromesso: lo segnala/redige E ne avvia rotazione/revoca** (un segreto esposto va CAMBIATO, non solo redatto); se ancora contenibile, redige. Poi continua. Insegna a chiudere il buco appena rilevato **e a contenere il blast-radius di ciò che è già uscito**.
...
- **Facet threat-model / hardening proattivo** (mining Stage-2 #14, 2026-07-10): un host/servizio **esposto** a internet senza hardening va trattato come **potenzialmente compromesso** (verifica i tentativi d'intrusione prima di fidartene; in dubbio, riformatta) — postura ops-security proattiva. E: **la sicurezza si completa per intero o vale meno di zero** — un hardening a metà dà falsa fiducia peggiore del nulla (completezza-di-sicurezza, cugino della completezza-dataset #21). Confine simmetrico anti-paranoia: proporzionale alla superficie d'esposizione e al valore dell'asset (non fortificare l'effimero). `[EXTRACTED]` dal mining.
