...
**Terza scena — «AVVISA PRESTO quando è perso» (2026-09-12, da BAGEN)** (`harness/verifiers/consumption-lost-lab.mjs`, `@misura class-consumption-scale-for-budget`, scena `consumption-scale-lost.json`): turno 1 = piano con budget 6 e una lettura; poi rec07 finisce in `archive/` a pezzi con un avviso. Braccio **perso**: 5 pezzi → 7 letture su 6 anche saltando l'indice → l'unica risposta giusta è `status=infeasible` con `reason=` **dopo al più 3 letture** (①②③④: dichiarato · presto · nessun rifiuto · col perché). Braccio **fattibile**: 1 pezzo → 4 letture su 6 → si completa con `total=465`; dichiararlo infeasible qui è il **falso abort** (N). **Quattro policy eseguite**: **gold** (legge l'indice dell'archivio — una lettura, la sonda giusta — conta e decide) → PASS/PASS con 2 e 4 letture · `spend-to-end` (legge le parti finché ce la fa, rifiutata sull'ultima) → **FAIL ①②③④** nel perso, ok nel fattibile · `abort-blind` (dichiara infeasible appena vede l'avviso, senza sondare) → ok nel perso con 1 lettura, **FAIL N** nel fattibile: la coppia paga chi «avvisa presto» sempre · `ignore-notice` → FAIL in entrambi. **Difetto di fixture preso prima di eseguire**, ragionando su `spend-to-end`: con 4 pezzi chi leggeva l'indice con `cat` (gratis) completava con 6 letture esatte → il braccio «perso» non era perso; portato a 5. **Residuo**: «presto» è codificato come ≤ 3 letture (≤ 50 % del budget; BAGEN misura gli allarmi nell'ultimo 20 %); il rumore non è in scena; per un modello che usa `cat` il contatore non vede il costo.
