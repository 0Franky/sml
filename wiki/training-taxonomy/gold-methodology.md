...
- **Predicato = QUANTIFICAZIONE, non esempio** (dal review decomposition 2026-06-29): se l'oracolo verifica una proprietà *"per tutti"* (es. "il core è comune a TUTTE le foglie"), il predicato deve **quantificare su tutti gli elementi** (`∀ stmt ∈ core: muovere(stmt→leaf_k) ⇒ ∃ test_j (j≠k) che rompe`), NON testare un singolo caso esemplificativo → altrimenti è **gameabile** (il modello soddisfa l'esempio noto e viola il resto).
- **Pinna il detector concreto**: per proprietà come "zero duplicazione" usa uno strumento *deterministico e pinnato* (es. `pylint --disable=all --enable=duplicate-code` con `min-similarity-lines=N` esplicito), MAI "un detector astratto / 3 alternative". Il valore di N può restare iperparametro ablabile, ma lo **strumento va scelto** (asimmetria col canonico = buco di ancoraggio).
- **Niente `sha256` su contenuto che passa per git** (dal verifier-run 2026-06-29): `git restore`/`git show` con `core.autocrlf` muta LF→CRLF → l'sha256 del file round-trippato ≠ originale (**non-portabile** Windows↔Linux). → per la preservazione-contenuto usa **oracoli semantici** (campi/struttura presenti), riserva `sha256` solo a copie che NON passano per git (es. un `.bak` diretto), e forza `git config core.autocrlf false` nel setup della fixture.
- **Fenomeni non-deterministici ≠ oracolo exit-code** (race: 20 trial → 3 esiti diversi, verificato): NON si validano con un pass/fail eseguito. → trasformali in **check STATICO sull'azione proposta** (es. "il modello NON emette una `>`-concorrente; emette append+lock"), deterministico.
...
Vedi [[../concepts/judge-design]] §coherence-anchoring-due-livelli: **NIENTE reward sul ramo**; penalità simmetrica via coerenza `campi-tipizzati↔facts-da-lane-fidate` (deterministica enum↔enum, pilastro — i `facts` vengono da `last_tool_calls`/`open_file_view`/…, **non** da una lane `env_facts`; vedi gold 6.2 §2bis) + `razionale↔campi` (L, complemento). Il pre-check deterministico = solo "contract ben-formato"; il "`conseguenze[]` non-vuoto quando ci sono conseguenze" è **giudizio L**, non pre-check.
