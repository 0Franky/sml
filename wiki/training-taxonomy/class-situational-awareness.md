...
**Skill radice** (imparata una volta, condivisa dalle figlie): **percepire e mantenere un modello accurato del proprio contesto operativo, e ancorarvi le decisioni** — *quando* siamo, *dove/con-cosa* sto operando, *rispetto a quale conoscenza*. Le figlie sono le tre **dimensioni** di questa situazione.

**Perché padre + figlie** (regola #20): le tre skill condividono il trigger di *grounding* ("prima di agire, orienta te stesso nella situazione reale") — impararlo UNA volta e poi specializzare *quale dimensione* della situazione ancorare (i) evita segnale ridondante, (ii) riflette la relazione reale (sono facce dello stesso orientamento), (iii) è composizionale ([[../concepts/compositional-curriculum-thinking-optimization]]).
