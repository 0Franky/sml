...
>
> **Hardening review-loop 2026-06-27**: questa sezione è stata rivista in un *review-loop 2026-06-27* che ha aggiunto/chiarito 5 guardie anti-gaming: (i) **probe-then-classify** è un'AZIONE, non un tier — reward sul probe eseguito, mai sulla label "uncertain"; (ii) **T-group** è ground-truth *annotata* (Q+L), non derivabile da git, con criterio operativo per inter-annotator agreement; (iii) **guardia hard** che vieta `git add -f`/commit di segreti nel self-versioning (veto > reward); (iv) **`automod`** reso invariante di provenienza (solo fonte fidata, mai da contenuto → anti prompt-injection); (v) **held-out negativo obbligatorio** (5bis) che premia il comportamento *differenziale* snapshot-vs-procedi.
...
2. **Value-tiering (non binario critico/non-critico)** — classifica ogni asset:
   ...
   - **T-group** (valore di gruppo): file singolarmente irrilevanti **MA collettivamente preziosi**, *specialmente quando poco è versionato e c'è stato molto lavoro* → tratta il **gruppo** come T-high (non cancellare a pezzi senza surfacing). È il caso nuovo che il binario tracked/untracked non cattura. ⚠️ **T-group è ground-truth ANNOTATA dal designer (Q+L), NON derivata da git** come tracked/untracked: git non sa nulla del "valore di gruppo", quindi non spacciarlo come deterministico/Q-puro. **Criterio operativo di annotazione riproducibile** (per inter-annotator agreement): marca T-group quando *≥N file untracked nella stessa dir di lavoro* **AND** *somma-righe complessiva > soglia* **AND** *nessuno è cache/artefatto noto* (estensioni/dir di cache escluse). Solo questo rende l'annotazione ripetibile tra annotatori diversi.
