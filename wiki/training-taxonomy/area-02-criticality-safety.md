...
- **Esempi**:
  ...
  - **(3) WRONG — awareness** — rinomina `parse()` modificando **solo** la definizione, lasciando i file dipendenti rotti (**3 file** importano `parse`: loader/cli/test). Label: *"sbagliato: cambiamento di simbolo senza dependency-check → riferimenti dangling."* Riconoscere. *(NB conteggio-oracolo = file-che-importano, non `grep \bparse\b` grezzo=7 occorrenze; vedi gold 3.2 verificato in sandbox 2026-06-29.)*
  - **(4) WRONG — recovery** — dopo il rename parziale l'import fallisce con **`ImportError: cannot import name 'parse'`** (NON `NameError` — `from X import parse` mancante fallisce all'**import-time** del modulo dipendente, prima di ogni uso runtime): rileva → `grep -rln` dei file che importano il vecchio nome → aggiorna tutti i call-site → import verde → memo "find-references PRIMA di rinominare". *(Corretto da NameError 2026-06-29, esecuzione sandbox gold 3.2.)*
