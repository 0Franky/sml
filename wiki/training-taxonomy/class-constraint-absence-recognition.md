...
> **Origine**: gap-scan 2026-07-24 su P1; difetto osservato in E-COMP ([[../harness-experiment-log]] §E-COMP, harness-experiment-log.md:365) — dare la mezza-skill "estrai il requisito load-bearing / dimensiona la risposta" tende a **peggiorare** il sapersi fermare (incatenamento-per-rito `3/12` vs `0/12` del controllo nudo, **sotto-soglia**, ipotesi coerente non fatto).
...
> Simmetria: il falso-positivo (dichiarare assenza quando un vincolo c'è) è penalizzato **quanto** il falso-negativo (fabbricare un vincolo quando non c'è). Nessun default fisso vince — specchio della simmetria cheap↔precious del padre (class-constraint-fit-decision.md:34) e del negativo "scelta di gusto → deferisci" di [[class-instruction-phase-clarification]] (class-instruction-phase-clarification.md:66).
