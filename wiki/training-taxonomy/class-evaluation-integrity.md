...
## PARENT / gerarchia (regola #20) — CEMENTATO

**Figlia di [[class-ground-truth-integrity]]**, **direzione CONTROLLATO** (io sono verificato), sorella speculare di [[class-independent-verification-integrity]] (CONTROLLORE):
...
- **Padre CEMENTATO 2026-07-11** (era PROPOSTO/#26 → **ratificato dall'utente**: "crea class-ground-truth-integrity e procedi"). L'**analisi-gap multi-agente** (67 candidati, 0 gap-super-padre sopravvissuti) ha confermato che `ground-truth-integrity` è una **radice con skill-radice REALE** (*"non-corrompere-la-verifica / àncora al ground-truth"*), NON un ombrello vuoto (#20): [[class-metacognitive-self-audit]] ("audita il TUO ragionamento") calzava male perché questa è **non-corrompere-un-valutatore-ESTERNO** (integrità/valori, non auto-audit). La sorella `independent-verification-integrity` è stata **RE-HOMED** alla stessa radice (#34, tracciato).
- **Radice-di-valori**: [[../feedback_reward_hacking_principle]] (#10) — cross-link di VALORI, non padre.
...
## Letteratura 2026 per l'hack-check (triage 2026-09-11 sera, [[../sota-techniques-catalog]] §RL-7 — verdetti sui riassunti dell'agente, PDF da leggere prima di costruire)
- **CapCode / CapReward** (arXiv:2606.07379) — **PDF letto il 2026-09-12**: in ogni caso di test si inietta un valore **casuale fra M equiprobabili** che il compito chiede di restituire insieme all'output: una policy onesta non può fare meglio di **1/M** in media (Appendice B.2), quindi un punteggio sopra il cap (test binomiale all'1 %) è **prova** di accesso ai test nascosti; rilevato in tre esposizioni (feedback, prompt, workspace) su agenti addestrati a barare, nessun falso positivo sulle policy oneste, ranking dei modelli conservato (τ di Kendall 0,94-0,98); **CapReward** trasforma il cap in reward a due tratti (cresce fino a B, decresce oltre) e non peggiora le policy oneste. **Limiti dichiarati**: solo test unitari predefiniti; il cheating «lieve» sotto il cap sopravvive; un agente che conosce il metodo può adattarsi. **Per noi**: i nostri oracoli sono assert su stato dei file, non test unitari — ma il principio si trasporta come **assert-canarino**: un assert *nascosto* che richiede un valore **casuale per run** (nessun run onesto può soddisfarlo) → se passa, il modello ha letto l'oracolo o il file degli assert. È rilevazione, non reward (#32). ⛔ Da costruire in `run-spec` (opzione `canary`) **dopo** la catena di sweep in corso (il runner non si tocca mentre gira): todo.
- **SpecBench** (arXiv:2605.21384): la metrica è il **gap fra test visibili e test composizionali held-out** — i modelli grandi saturano i visibili e falliscono gli hidden. È il nostro mutation test espresso come **numero per fixture**: utile come riga di reporting, non come reward (#32).
- **Reward Hacking Benchmark** (arXiv:2605.02964, già in catalogo RL-3), il fatto che serve qui: *l'RL post-training **aumenta** l'hacking, e il 72% degli exploit ha un razionale esplicito nel CoT* → la catena ancorata (#10) non è solo anti-shortcut: è anche il posto dove l'hack **si dichiara**, quindi vale come segnale di detection nel reward-side.

## Coherence-audit (playbook §5) — auto-check
1. Struttura-sezioni ✓ · 2. Reward outcome-anchored (passa-immutato + causalità + verifier-side) + hack-check + simmetria + ablazione-lato-S ✓ · 3. **Home = area-07** (Tier-X integrità adversariale) con **split #11 esplicito** (F-difesa-esiste / S-disposizione-nuova) e **§Cosa-esisteva-già** che LINKA (non duplica) area-05/13/14 ✓ · **padre CEMENTATO = [[class-ground-truth-integrity]]** (ratificato utente 2026-07-11; sorella speculare independent-verification-integrity RE-HOMED alla stessa radice; confermato da analisi-gap) ✓ · 4. Gold/fixture self-contained + decontaminato (#22/#18) ✓ · 5. Transfer A/B/C ≥3-4 non-tecnici, dal banale al sistemico (#19) ✓ · 6. Negativi N1-N6 **simmetrici** (N1-N5 manomissione / N6 over-caution) + positivi P1-P3 enumerati (#21) ✓ · 7. Integrità fattuale (nessun claim-perf inventato; riusa oracoli reali `verifiers/` + causalità da area-13) ✓ · 8. Nessuna contraddizione: **confine netto vs [[class-independent-verification-integrity]]** (verifico-altri vs sono-verificato) e vs le difese-F per-area (skill-S vs meccanismo-F) ✓ · 9. Wiring: index + area-07 + sorella + area-05/13/14 cross-ref + playbook + todo + log ⏳ (in corso) · 10. Caveat → §4 playbook `[REWARD-INTEGRITY]` ⏳. **STORIA**: germogliata dal revert testo-only di [[class-code-optimization]] (msg 1658) → proposta TG 1668 → **approvata "crea"** → creata regime agentico + split #11 + sorella-di-independent-verification → **padre CEMENTATO [[class-ground-truth-integrity]]** (split ratificato 2026-07-11 dopo analisi-gap multi-agente: 67 candidati, 0 gap-super-padre; struttura finale = 3 radici-sorelle cross-linkate self-audit/situational-awareness/ground-truth-integrity). ⏳ **wiring index DEFERITO** (index.md ha modifiche non-committate di una sessione parallela → non entangло; tracciato in todo).
