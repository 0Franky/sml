...
- **[REWARD-Q] Ottimizzazione/efficienza (dal build-out [[class-code-optimization]])**: correttezza **behavior-preserving** (public+hidden+property, NON "stessi test-verdi") = cancello DOMINANTE · **proxy DETERMINISTICO** primario (conteggio-istruzioni/#alloc/#IO, riproducibile solo per prog. deterministici) + **wall-clock STATISTICO** secondario (warmup+mediana-K+significatività, mai oracolo unico) · harness/test **CONGELATI** verifier-owned (anti-tamper, come area-05) · ② attribuzione = **per-hunk-ablation** (multi-hunk, verifier-side non-circolare) / **MCQ-controfattuale** (hunk-singolo), MAI "collo-giusto" circolare né previsione-copiabile · **race → stress/TSan** (non property-single-pass) · **parallelizzazione → critical-path/wall-clock** (il conteggio-istruzioni SALE con l'overhead-thread) · regime-proxy fissato dalla **fixture** non dal modello. — [[class-code-optimization]] (build-out di area-14, confine vs foglia efficiency fixed-limit).
