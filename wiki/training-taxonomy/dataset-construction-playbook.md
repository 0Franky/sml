...
- **[REWARD-Q] Ottimizzazione/efficienza — regime TESTO-ONLY (dal build-out [[class-code-optimization]], redesign msg 1654)**: il modello emette **solo testo** (codice-ottimizzato + ragionamento); il **VERIFICATORE** esegue e vota → **anti-tamper, measure-then-declare e auto-strumentazione impossibili PER COSTRUZIONE** (non guardrail). Correttezza **behavior-preserving** (public+hidden+property, NON "stessi test-verdi") = cancello DOMINANTE · **proxy DETERMINISTICO** primario (conteggio-istruzioni/#alloc/#IO, riproducibile solo per prog. deterministici) + **wall-clock STATISTICO** secondario (mai oracolo unico) · ② attribuzione = **sola MCQ-controfattuale** (non-gameable: si sceglie, nessun numero da copiare; **rimossi** ablation-per-hunk e previsione-quantitativa, che erano vettori-di-hack del regime agentico) · **race → stress/TSan** · **parallelizzazione → critical-path/wall-clock** (il conteggio-istruzioni SALE con l'overhead-thread) · regime-proxy fissato dalla **fixture** non dal modello · unico hack di misura superstite = **benchmark-gaming** (hardcode) → held-out+taglie+worst-case. **Capacità agentica "profila-poi-ottimizza" → relocata a Fase-3 RL** (#34). Il testo-only è l'applicazione più forte del principio RIMUOVI-L'INCENTIVO (rimuove la CAPACITÀ). — [[class-code-optimization]].
