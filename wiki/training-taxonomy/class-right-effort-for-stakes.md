...
**Letteratura per il reward di sforzo (triage 2026-09-11, PDF letto il 2026-09-12 via riassuntore)** — *Correct, Concise and Complete: Multi-stage Training for Adaptive Reasoning* (arXiv:2601.02972): reward `R = R_C − λ·(L(y) − L(y_first))/L(y)`, λ = 1 — si penalizza la **frazione di traccia dopo la prima risposta corretta** (zero penalità se nessuna risposta è corretta, così non si premia il fermarsi presto e sbagliato); SFT sul più corto fra 8 campioni corretti + GRPO; Qwen3-8B/32B dense: −28 % di lunghezza per −1,6 punti medi (8B), −40 % sul 32B; la lunghezza **cresce con la difficoltà** (adattività misurata su MATH-500 per livello). Perimetro: matematica-only, single-answer, nessun task agentico. **Trasporto alle scene**: l'analogo di «prima risposta corretta» è **t\*, il primo turno in cui gli assert dello stato sono già soddisfatti**; tutto il lavoro dopo t\* (tool call, turni, token) è **overhead misurabile con gli oracoli su file, senza giudice** — e la difficoltà del braccio (scarso/abbondante, k del rumore) dà la curva di adattività. ✅ **COSTRUITO il 2026-09-15**: `run-spec` valuta gli assert **alla fine di ogni turno** (opzione `perTurnAsserts`, accesa di default in `eval/run-scene.mjs`, `EVAL_TSTAR=0` per spegnerla) e restituisce `tStar` · `turniDopoTStar` · `perTurnAsserts`; la tabella di `run-scene-batch` porta la colonna **turni-dopo-t***. Non è un reward per-esempio sul campo «sforzo» (#32): è un input misurato a valle dell'esito.

> ⚠️ **Due vincoli scoperti costruendolo, ed è il motivo per cui non era un ciclo banale.** **(1) Gli assert NON sono innocui**: in `module-boundary-gate` un assert riscrive `cfg/consent.txt` e lancia `bin/run-all.sh` — valutarli a ogni turno nella workdir vera **cambierebbe il mondo sotto i piedi dell'agente**, cioè la misura distruggerebbe ciò che misura. Si valutano su una **copia** dello stato. *(Mutation test: valutandoli nel mondo vero, l'agente al turno 2 vede `99` invece di `0` — il test lo pretende e diventa rosso.)* **(2) Il mondo avanza fra i turni**: prima dell'ultima mutazione un assert può essere vero **per pura assenza del fatto** (nel canary *«nessun BUG in produzione»* è vero finché il BUG non arriva) → **t\* si conta solo dai turni ≥ l'ultima mutazione**; il grezzo resta leggibile come `primoTurnoTuttiVeri`, ma **non si chiama t\***. ✅ **Catena `run-scene` → t\* provata end-to-end con un modello VERO** (`qwen3.6-27b`, turni tagliati a 20 s): braccio `banale` → `tStar = 1`, `turniDopoTStar = 0`; braccio `vale-la-pena` → `tStar = null` (il lavoro non è mai finito). ⚠️ **Correzione di quanto avevo appena scritto**: credevo fosse una prova **a spesa zero** perché avevo messo una chiave invalida in `OPENROUTER_KEYS` — **non è vero, è costata 0,054 \$**. `eval/env-keys.mjs` legge **solo il file `.env`** e non guarda l'ambiente: quella variabile non disarma niente. La lezione vale più della cifra: **l'unica prova di «non ha speso» è il credito prima/dopo**, non la convinzione di aver disarmato qualcosa. Test: `harness/test/integration/run-spec-tstar.test.mjs` (6 prove, rosso prima).
