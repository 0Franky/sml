...
**La barra (anti-cerimonia).** L'intuizione è qui, ma va citata con onestà statistica (#0/#22/#35b) — **E-COMP NON è un difetto documentato**: è un **segnale SOTTO-SOGLIA, INCONCLUSO**. Dare la mezza-skill *"estrai il requisito load-bearing / dimensiona la risposta"* *sembrava* peggiorare il sapersi fermare (incatenamento-per-rito `3/12` arm-1 **e** `3/12` arm-4 vs `0/12` del controllo nudo), ma la **soglia pre-registrata `6/12` NON è superata** e il denominatore reale è **n=8** (repliche correlate, non 24) → **ipotesi meccanicistica coerente ma NON dimostrata, da RI-TESTARE** con più negativi prima di usarla (harness-experiment-log.md:353-354). Non è una prova portante: è la **direzione plausibile** che *motiva* la barra. La barra vera è **indipendente da E-COMP**: se il reward premia *"produci sempre un ordine elaborato / argomenta sempre"*, crea l'hack *"aggiungi cerimonia"* → il reward qui è ancorato all'**esito reale** (danno contenuto? destinatario in grado di agire?), MAI alla forma; e il **GATE §Gate** (policy-stupida) è la prova che la forza bruta perde.
...
**Falsificabile perché**: (a) l'ordine è materializzato in una **partizione ordinata** e la conseguenza è un **fatto eseguibile sulla fixture** (danno accumulato, dipendenze violate — veri-per-costruzione, regola #22); (b) il registro è valutato per-esempio SOLO sul **non-ramo**, ma quel non-ramo è **GRADUATO, non binario**: non "il destinatario riesce ad agire? sì/no" (un `6` sepolto in 4 paragrafi lo soddisferebbe), bensì `azionabilità ∈ [0,1]` = **con quanta accuratezza E con quanta latenza** il destinatario simulato compie l'azione corretta, dove accuratezza/latenza **DEGRADANO col volume irrilevante** → l'over-structure **perde PER-ESEMPIO** (specularmente all'under-structure che lascia il destinatario senza il perché). Il gradiente pesa il **costo-di-estrazione del segnale utile** (rumore-da-filtrare), **NON** la lunghezza in sé: un testo lungo ma tutto load-bearing su posta alta non è penalizzato (anti reward-sulla-brevità). Il **determinante-del-ramo** (registro↔posta) resta al **distribuzionale** (calibrazione/ECE, non per-esempio — #32, vedi §Reward).
...
oracolo (metrica unificata): outcome = total_damage + late_penalty·(turni_spesi − turni_minimi) ; dependency_violations = #azioni-eseguite-prima-del-loro-depends_on
             (total_damage = #turni-prima-di-B ; qui tutte e 4 le azioni sono necessarie → late_penalty=0)
gold_order = [B, D, C, A]  → total_damage=0, violations=0, late_penalty=0
```
...
- **N2 — sequenza senza contenimento-necessario → riordinare/triage NON è gratis (COSTO ESEGUIBILE).** Fixture: "prepara il caffè per l'ospite che sta arrivando: scalda l'acqua, macina i chicchi, versa" — nessuna azione contiene un danno, l'ordine dato è già valido, le dipendenze reggono. **Denti (senza cui N2 è decorativo)**: l'`outcome` unificato è `total_damage + late_penalty·(turni_spesi − turni_minimi_necessari)` — ogni azione **consuma un turno** sotto un **clock** (l'ospite aspetta). Un **triage/diagnosi inserito per rito** ("prima controlla la qualità dei chicchi", "verifica la temperatura ideale") **non riduce alcun danno** (non ce n'è) **ma consuma un turno** → `outcome` **peggiore** del semplice eseguire l'ordine dato. **Gold**: eseguire l'ordine valido con il **triage INTERAMENTE ASSENTE**; l'oracolo gronda `late_penalty` sui turni-non-necessari → il riordino/triage-per-rito **perde PER-ESEMPIO**, non solo distribuzionalmente. (Over-reorder = l'analogo-sequenziale dell'over-decomposition, gold-example-decomposition.md:421; qui col **costo eseguibile**, non solo dichiarato.)
...
- **R-neg — over-structure sul banale (SIMMETRICO).** Fixture identica a R1 ma il modello produce 4 paragrafi con heading. **Gold**: FAIL **PER-ESEMPIO** (azionabilità graduata: il `6` corretto è **sepolto** → il destinatario simulato lo estrae con più latenza/errore = score più basso, l'esaustivo produce RUMORE) **E** distribuzionale (registro-mismatch/ECE). Specchio esatto di R1: over- e under- mordono allo **stesso livello** (per-esempio), non uno per-esempio e l'altro solo in aggregato.
...
| **P-ord-2**: "metti sempre contenimento/triage per primo (riordina sempre)" | Muore su **N1**: il contenimento drastico (interruttore generale) ha `collateral=high` → `danno_totale` peggiore del secchio → **FAIL**. E su **N2** (nessun contenimento serve): il triage-per-rito **consuma un turno sotto il clock** → `outcome` peggiore dell'eseguire-l'ordine-dato → **FAIL** (costo eseguibile per-esempio, non solo dichiarato). |
| **P-reg-1**: "argomenta/struttura sempre (registro massimo)" | Muore su **R1/R-neg** sia **PER-ESEMPIO** (azionabilità graduata: la risposta corretta sepolta = score più basso) sia **distribuzionalmente** (ECE registro↔posta): over-elaborazione sistematica + risposta sepolta → **FAIL**. |
...
**Nessuna policy fissa passa l'insieme bilanciato** → il reward discrimina il giudizio. Il **budget reale che l'esaustivo sfonda, tutto PER-ESEMPIO**: (ordine) il contenimento-drastico-di-riflesso **DANNEGGIA** (N1, danno collaterale) e il triage-per-rito **CONSUMA TURNI sotto il clock** (N2, `late_penalty`); (registro) l'over-argomentazione **produce RUMORE** che un destinatario simulato paga per filtrare (R-neg, azionabilità graduata). Le cure del GATE sono entrambe presenti e mordono per-esempio, non solo in aggregato.

## §3 — Reward (ANCORATO all'OUTCOME — #10) + difesa #32

**(a) Ordine — Q, per-esempio sull'OUTCOME (legittimo):**
- `score_ordine` = funzione decrescente dell'`outcome` unificato `total_damage + late_penalty·(turni_spesi − turni_minimi)` **e** di `dependency_violations`, calcolati **eseguendo l'ordine scelto nel world_model della fixture** (danno accumulato, turni consumati da passi non-necessari, dipendenze violate). Questo gronda la **CONSEGUENZA dell'ordine**, non una label "ordine-giusto": è un **outcome simulato** (come [[class-consequence-intention-conflict]] gronda *#richieste ≤ baseline* e [[class-anticipation-and-irreversibility]] gronda il *match-partizione* eseguito, non un campo). Il modello deve **DERIVARE** l'ordine dai campi (`stops_damage`, `depends_on`, `collateral`) — non echeggiarne uno. Il termine `late_penalty` è ciò che dà **denti PER-ESEMPIO all'over-reorder** (N2): un triage-per-rito su fixture-senza-danno consuma turni → outcome peggiore.
...
- **Per-esempio si gronda SOLO il non-ramo** (input/outcome genuini, ⊥ alla scelta-di-registro): (i) **correttezza** della risposta (il "6" è giusto; l'azione suggerita risolve); (ii) **stato-di-conoscenza del destinatario simulato, come SCORE GRADUATO** `azionabilità ∈ [0,1]` — NON il binario "riesce ad agire?" (che un `6` sepolto in 4 paragrafi soddisferebbe), ma **accuratezza dell'azione compiuta penalizzata dalla latenza/rumore di estrazione**: la risposta corretta **sepolta** sotto volume irrilevante abbassa lo score (il destinatario paga per filtrare) **esattamente come** la risposta **troppo secca** che gli toglie il perché → **entrambi i poli** (over-/under-comunicazione) mordono **PER-ESEMPIO**, non solo in aggregato. Il gradiente pesa il **rumore-da-filtrare**, mai il conteggio-caratteri (un lungo tutto-load-bearing su posta alta non è penalizzato). **DA COSTRUIRE (#11)**: l'oracolo-destinatario graduato (accuratezza+latenza sensibili al rapporto segnale/rumore) è parte dello scaffold **verifier-sandbox ancora pending** — oggi format-only ([[../decisions/2026-06-23-pi-harness-base]]).
...
**Simmetria** (#21): penalità pari per over- e under- su entrambe le skill, e ora **entrambi i poli mordono PER-ESEMPIO** (non uno per-esempio e l'altro solo in aggregato): over-contieni ↔ non-contieni (N1 danno collaterale ↔ danno-non-fermato), over-reorder ↔ ordine-sbagliato (N2 `late_penalty` ↔ `total_damage`/`violations`), over-argomenta ↔ rispondi-di-getto (R-neg azionabilità-graduata ↔ R2/R3 under-register). Nessun default fisso vince (dimostrato in §Gate).
...
- **S (skill nei pesi) — dominante.** Derivare l'ordine dai campi (contenimento+dipendenze+reversibilità) e scegliere il registro sono **ragionamento**. Stato-senza-training: **DEGRADATA-MA-UTILE** — il base a volte ordina in modo sensato ma non **affidabilmente** mette il contenimento-prima sotto distrattori, e **over-elabora di default** (che il registro non calibrato sia un modo-di-fallimento è un'**ipotesi SOTTO-SOGLIA da ri-testare, non un fatto**: E-COMP `3/12`+`3/12` vs `0/12`, soglia `6/12` non superata, n=8, INCONCLUSIVO — harness-experiment-log.md:353-354). È la metà che il training internalizza.
...
**Label-generation:**
- **(a) Ordine — oracolo strutturale self-contained** (riusa il pattern [[../../harness/verifiers/async-schedule-gen]]): ogni fixture modella `azioni[]` con `stops_damage?`, `damage_rate`, `depends_on`, `reversible?`, `collateral`; un `world_model` che accumula danno per turno-a-perdita-aperta **e** `late_penalty` sui turni consumati da passi non-necessari; `score` = f(`outcome` unificato, dipendenze_violate) dell'ordine scelto. **Veri-per-costruzione** (#22). Presentazione **scrambled** (anti P-ord-1). **PROPORZIONE FISSA come vincolo di label-gen** (il gate ORDINE regge SOLO se il mix è genuinamente bilanciato, NON 90/10): target **≈ 1/3 serve-contenimento : 1/3 N1 over-containment-danneggia : 1/3 N2 nessun-contenimento** (± tolleranza dichiarata), **verificato dal generatore prima di emettere lo split** — un mix sbilanciato verso *serve-contenimento* farebbe passare P-ord-2 (contieni/triage-sempre) in aggregato, quindi la proporzione è un **invariante di costruzione**, non un'aspirazione (anti P-ord-2).
- **(b) Registro** — fixture con `{richiesta, posta, ambiguità, destinatario, fatto/azione-corretta}`; **destinatario simulato GRADUATO** (oracolo di azionabilità `∈[0,1]`: accuratezza+latenza sensibili al rumore — **DA COSTRUIRE #11**, scaffold verifier-sandbox pending) per il non-ramo per-esempio; **held-out bilanciato con PROPORZIONE FISSA ≈ 1/3 banale : 1/3 azione-necessaria : 1/3 alta-posta** (± tolleranza dichiarata, verificata dal generatore) + **ECE** per il determinante-del-ramo (#32) — anche qui uno split sbilanciato lascerebbe passare P-reg-1/P-reg-2 in aggregato. Difficoltà anti-cue: la posta **non** leggibile da una parola-chiave (un "urgente!" su una richiesta banale; una richiesta calma ad alta posta reale).
...
- **Over-containment / over-reorder** (mettere il drastico o riordinare/triage per riflesso) → neutralizzato **PER-ESEMPIO** da **N1/N2**: l'oracolo conta il danno collaterale (N1) **e la `late_penalty` sui turni consumati da un triage-per-rito** (N2, costo eseguibile), non solo distribuzionale.
- **Registro-massimo-sempre** / **registro-minimo-sempre** → neutralizzati da §Gate **PER-ESEMPIO**: il massimo perde sull'azionabilità graduata (risposta corretta sepolta = score più basso) **E** distribuzionalmente (ECE); il minimo perde sullo stato-di-conoscenza del destinatario. Entrambi i poli allo stesso livello.
