...
## §6 — Raffinamento F14: il meccanismo-faller è NARROW e il modo-di-fallimento è "rileva-ma-molla" (2026-07-05) [EXTRACTED]
Costruito un set di 4 task **puramente sign/wrap** e validato girando flash-lite (ours@keep6):
- `total_minutes` (durata overnight) **casca**; `hours_after`(%24), `compass_diff`(min(d,360-d)), `weekday_gap`(%7) **NON cascano** — il modello scrive la fix idiomatica giusta.
- **Insight [EXTRACTED]:** l'aritmetica modulare **IDIOMATICA** (orologio/bussola/calendario) è **memorizzata** → non è un buco. Cascano solo i wrap **DISGUISED**: dove il naive è una sottrazione/computo che va **NEGATIVO** in un framing che **NON segnala "modulo"**, e la fix è **non-idiomatica**. Due faller confermati: `total_minutes` (durata→negativa, framing "turni") e **#145** (input negativo→segno nella digit-sum, framing "ordina per somma cifre").
