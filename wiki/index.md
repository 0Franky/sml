...
- [`training-taxonomy/templates/`](training-taxonomy/templates/) 🆕🧬 — **sistema template-inheritance** (motore del rollout gold): **2 template di gruppo** — `area02-group1-destructive.template.md` (1.x distruttive, 4 leaf-delta 1.1-1.4) + `area02-group3-preflight.template.md` (3.x pre-flight, 26 slot, leaf-delta 3.1/3.2/3.3) — + `expand.py` + `EXPANSION.md` → producono i `gold-example-area02-*.expanded.md` (gold canonici di training, **byte-verificabili**). Pattern validato 360° (byte-equal round-trip su 1.1 e 3.2 + generalizza ai sibling + fix-once-propaga); oracoli **eseguiti verdi** 2026-06-29 (6/6 spec 1.x + sandbox git/disco per 3.x)
