...
- [~] 🆕 **PRIMO artefatto template-inheritance** (FOUNDATIONAL, 2026-06-29): `wiki/training-taxonomy/templates/area02-group1-destructive.template.md` (skeleton + **§ORACOLO-PATTERN O1-O7** + §REWARD-PATTERN promossi a invariante-di-famiglia) + leaf-delta `1.1-delete`/`1.2-overwrite` (coi fix 1.2 incorporati: field-presence, sha256-solo-.bak, autocrlf-false, race→statico) + `EXPANSION.md` (round-trip expand(delta-1.1)≡canonico). **UNDER agnostic review**. Aperti: slot-granularity, delta-size ~95-115 vs target 50-100, **byte-verificabilità dell'espansione**, possibile **O8 scope-containment** per foglia 1.4-side-effect. → se regge = pattern del rollout 215-foglie.
