...
- [~] 🆕 **Foglia-esercizi `hierarchical-decomposition`** (idea utente msg 284, "deve esserci nel 1° lotto + includi la tua miglioria" msg 287): concept `wiki/concepts/hierarchical-decomposition.md` ✓. **DRAFT scritto** `gold-example-decomposition.md` (scenario: rifattorizza N handler in gerarchia; reward **materializzato in codice** = pytest-coverage + duplicate-detector→0 + **mutation-probe** per l'invariante-nel-core; anti-over-decomposition **differenziale** su 5a banale/5b falso-invariante). **UNDER agnostic review**. → **wiring pending** (index/log/area-file); TODO: **pinnare un duplicate-detector concreto + soglia-N**; verificare link `wrapper-context-assembly`/`error-memo`; decidere `area:` (ora area-03) e reward_tag Q+L. Classif: S + piccola F-scaffold.
