...
>
> **Dov'e' cablato oggi**: `self-sealing-A1-canary` e `self-sealing-noise` (reward ① ← *il canary e' spento*, in entrambi i bracci del canary) · `module-boundary-gate` (①-tenuta ← ①-controllo, in entrambi i bracci) · `retroactive-noise-hook` (*ricollegato* ↔ *non neutralizzato* si controllano **a vicenda**; nel braccio «ritira» *disarmato* ← *hook ancora sano*). ⚠️ **Residuo dichiarato**: dove il controllo vive nel braccio **gemello** (`consumption-scale-lost`) il campo non lo esprime — li' l'unita' di lettura resta la **coppia**, e vale il paragrafo qui sopra. ⚠️ **E su `design-artifact` la nota del 12 era sbagliata**: avevo scritto che lì il controllo non serve perché « il reward stesso esige il lavoro fatto ». Falso per ④ (*la deviazione è segnata*): se il modello **non tocca mai il documento**, la riga `validazione: validate.py` del turno 1 e la sezione delle decisioni sono ancora lì e ④ passa **per assenza** — il suo controllo è ① (*il documento dice `parse.py` ORA*), e ora è dichiarato (`control: 1`, verificato eseguendo: una policy che applica il vincolo al codice e lascia il documento del turno 1 dà `perAssenza=[2]`). In `consumption-scale-midway` invece il campo non c'è perché davvero non serve: il reward esige il lavoro fatto.
