...
// per ogni turno i l'agente (se dato) agisce con TURN=i e TURNS=n nell'ambiente, e subito dopo si
// applicano le mutazioni dichiarate `after_turn: i`. Gli assert si valutano ALLA FINE.
 ...
 * @param {object} opts  { agent?: string } — bash eseguito a ogni turno con TURN/TURNS nell'ambiente
 * @returns {{ passed, setupError, results, turnsRun, agentErrors }}
 */
export function runScene(spec, opts = {}) {
  ...
  try {
    ...
    if (!setupError) {
      ...
      for (let i = 1; i <= nTurns && !setupError; i++) {
        if (opts.agent) {
          const r = sh(opts.agent, dir, { ...process.env, TURN: String(i), TURNS: String(nTurns) });
          // Un agente che fallisce è un DATO della scena (es. deploy bloccato), non un errore del runner.
          if (r.exit !== 0) agentErrors.push({ turn: i, exit: r.exit, stderr: r.stderr.slice(0, 400) });
        }
  ...
  return { passed, setupError, results, turnsRun, agentErrors };
}
