...
| `secrets-guardrail.ts` | tool `add_secret`/`list_secrets`/`request_secret` + `tool_result`/`tool_call` | **Lato EGRESS dei segreti.** Redige output che matchano i pattern statici (incl. **Google `AIza…` = GEMINI_API_KEY**) **+ la secrets-map dinamica** (riferimenti opachi per-sessione, in-memory) + **sealed-secrets** (injection `{{secret:NAME}}` sink-gated al `tool_call`, mai il valore al modello). Logica in `src/secrets-redact.mjs` + `src/sealed-secrets.mjs`. |
