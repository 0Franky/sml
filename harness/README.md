...
| `secrets-guardrail.ts` | tool `add_secret`/`list_secrets`/`request_secret`/`request_local_http` + `tool_result`/`tool_call` | **Lato EGRESS dei segreti.** Redige output che matchano i pattern statici (incl. **Google `AIza…` = GEMINI_API_KEY**) **+ la secrets-map dinamica** (riferimenti opachi per-sessione, in-memory) + **sealed-secrets** (injection `{{secret:NAME}}` sink-gated al `tool_call`, mai il valore al modello). `request_local_http` = il modello CHIEDE (Ask TUI), l'utente acconsente, per usare un secret locale su http-loopback. Logica in `src/secrets-redact.mjs` + `src/sealed-secrets.mjs`. |
| `regex-ingress.ts` | `input` (transform) | **Lato INGRESS dei segreti — estensione SEPARATA, disattivabile** (`secrets.regexIngress=off` o cancella il file). Intercetta valori secret-shaped nell'INPUT utente PRIMA del provider → sigilla + sostituisce con `{{secret:INGRESS_N}}` (il valore non entra nel `<context>`, mai al provider/transcript). Più invasiva (muta ogni input) → tenuta a parte da `secrets-guardrail`. **Soft-dep**: standalone = anti-provider; +`secrets-guardrail` = anche anti-transcript. Logica in `src/sealed-secrets.mjs` (`autoSealIngress`). |
...
- **Env** (override rapido, vince sul file): `HARNESS_TOKEN_MATRIOSKA_PCT`, `HARNESS_TOKEN_REORDER_PCT`, `HARNESS_WATCH_MATRIOSKA`, `HARNESS_WATCH_REORDER`, `HARNESS_MAX_DEPTH`, `HARNESS_OUTPUT_RESERVE_PCT`, `HARNESS_MESSAGES_WINDOW_N`, `HARNESS_MESSAGES_EXCLUDE_CURRENT_TURN`, `HARNESS_GATHERING_MODE`, `HARNESS_GATHERING_MIN_TASKS`, `HARNESS_AUTOFOCUS_MODE`.
...
- **`messagesExcludeCurrentTurn`** (default `true`): Strada-2 complementarità (P1-B) — esclude il TURNO CORRENTE dalla lane `<messages_with_user>` perché la `native-window` (keepTurns=1) lo porta già → niente doppia-chat (overlap=0). Mettilo a `false` solo se alzi `keepTurns` nella native-window o per debug. Env `HARNESS_MESSAGES_EXCLUDE_CURRENT_TURN`.
...
- **`secrets.sinkGating`** (default `strict`) + **`secrets.regexIngress`** (default `ask`): sealed-secrets — `sinkGating` regola l'uso di `{{secret:NAME}}` (strict=allow-host fail-closed + https-only, warn, off); `regexIngress` (off|ask|auto) cattura nell'**input** i valori secret-shaped e li sigilla+trasforma PRIMA che vadano al provider (ask=notifica, auto=silenzioso). Env: `HARNESS_SECRETS_SINK_GATING`, `HARNESS_SECRETS_REGEX_INGRESS`. Vedi §"Sealed secrets".
...
- **`allowLocalHttp`** (per-secret, default OFF): per i secret **LOCALI** (es. un JWT di sessione contro il tuo dev-server) che devono andare su `http://localhost`. Quando ON, il secret è iniettabile su http **MA SOLO verso loopback LETTERALE** (`localhost`/`127.0.0.1`/`127.x.x.x`/`::1`) — mai verso host esterni in http (restano https-only), e il fast-path è **bloccato se c'è host-pinning** (`--resolve`/proxy che rimapperebbe `localhost` su un IP esterno). **Chi lo abilita** (mai il modello da solo): l'utente da CLI (`set-secret … --allow-local-http`) / config (`"allowLocalHttp": true`), **oppure** il modello chiama `request_local_http` → ti compare un **Ask** nella TUI e tu accetti intenzionalmente (in headless degrada a notify + abilitazione out-of-band). Razionale: il rischio "chiaro sul filo" non esiste verso loopback (il traffico non lascia la macchina).
- **`regexIngress`** (cattura di valori secret-shaped incollati, default `ask`): un valore che SEMBRA un secret (`sk-`, `AIza`, JWT, `github_pat`…) incollato nell'input viene **intercettato nel hook `input`, sigillato e sostituito con `{{secret:INGRESS_N}}` PRIMA di raggiungere il modello/provider** (e quindi anche fuori dai transcript nativi). `ask` notifica l'utente, `auto` è silenzioso, `off` disattiva. I match auto-rilevati restano in lockdown (no `allowedSinks`): un paste accidentale non è esfiltrabile.
...
> Logica node-pure in `src/sealed-secrets.mjs` (test 126/126, incl. red-team review-loop ×4 su allowLocalHttp — host-normalization curl-accurate via `new URL`, incl. dot %2e/unicode). Wiring su DUE estensioni: `.pi/extensions/secrets-guardrail.ts` (lato EGRESS: redazione + injection sink-gated) e `.pi/extensions/regex-ingress.ts` (lato INGRESS: cattura dall'input utente — separata e disattivabile, `secrets.regexIngress=off`). Soft-dep: regex-ingress standalone = anti-provider; +secrets-guardrail = anche anti-transcript.

> **📋 Nota — residui noti & lavori futuri (sealed-secrets / `allowLocalHttp`).** L'invariante PRIMARIO (il valore non raggiunge mai il provider LLM né i transcript) è garantito strutturalmente all'iniezione `tool_call` (post-provider) ed è indipendente da tutto ciò che segue. I punti sotto riguardano la difesa SECONDARIA *exfiltration-via-use* (de-prioritizzata): un modello indotto che prova a far uscire il segreto da un sink. Onestà sui limiti (cfr. design completo: [`wiki/concepts/sealed-secrets.md`](../wiki/concepts/sealed-secrets.md) §4bis).
> - **Residui accettati** (de-prioritizzati, fail-safe): (a) host **single-label** senza punto (`curl http://localhost myserver`) → esfiltra solo con DNS-locale controllato dall'attaccante (contrived); (b) **relay/SSRF su un server loopback** → il segreto raggiunge localhost legittimamente, poi un servizio locale potrebbe inoltrarlo (inerente al permettere l'egress-loopback); (c) **consenso per-sessione** (non per-operazione): un `request_local_http` approvato vale per tutta la sessione e ogni servizio locale; (d) **URL/`://` dentro un arg quotato** → falso-blocco (fail-SAFE: usa https o semplifica); (e) il fast-path loopback **bypassa gli `allowedSinks`** dichiarati dal secret (intenzionale per un JWT locale).
> - **🎯 Direzione durevole (ADR futuro)**: il fast-path che fa parsing-regex+`new URL` di un comando shell free-form è fragile by design (il red-team l'ha bucato 4 volte). Il target è iniettare i secret SOLO in un **tool `http_request` tipizzato** controllato dall'harness (`url` typed → 1 `new URL` → check loopback; niente shell) → elimina `hasForeignHostToken`/`hasCommandComposition`. Il regex-fast-path resta **interim/fallback**.
> - **Polish post-ship**: per-call confirm (vs per-sessione), unit-test del tool `request_local_http`, test-label hardening.
...
- **Fase 1** ✅ (state layer + wiring, typecheck-GREEN + tutti gli smoke verdi):
  ...
  - **pi-extensions wired** (16, typecheck-green): `context-assembly` · `native-window` · `vars-queue` · `nested-compact` · `checkpoint` · `turn-trace` · `error-memo` · `secrets-guardrail` · `regex-ingress` · `pre-flight` · `verifier-sandbox` · `contradiction-detection` · `conversation-capture` · `var-ops` · `sliding-var` · `gemini-compat`.
