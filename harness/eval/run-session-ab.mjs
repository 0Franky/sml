...
async function main() {
  ...
  for (let ci = 0; ci < configs.length; ci++) {
    ...
    report.configs.push({ label: cfg.label, arm: cfg.arm, keep: cfg.keep, nExt: res.nExt, hasContext: res.hasContext, passed, graded, apiErr, passPct, totTurns, finalTokens: res.finalTokens, userTurnsRecorded: res.userTurnsRecorded, evictionOrdinal: res.evictionOrdinal, noteCalls: res.noteCalls, jotCalls: res.jotCalls, setVarCalls: res.setVarCalls, saveToolCalls: res.saveToolCalls, taskDigestFacts: res.taskDigestFacts, probe: { ...probe, turns: res.probe?.turns, apiError: res.probe?.apiError }, probeAnswer: res.probe?.answer, perTask }); // F22: forwarda le guardie di regressione (userTurnsRecorded/evictionOrdinal) + F23: conteggio salvataggi (saveToolCalls) dal worker
