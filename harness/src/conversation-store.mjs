...
export class ConversationStore {
  ...
  constructor(dbPath = ":memory:", opts = {}) {
    ...
    // busy_timeout PRIMA di WAL e dello SCHEMA (bug P0 2026-07-04): con WAL due writer concorrenti (2 processi pi,
    // o driver+TUI) e busy_timeout=0 → "database is locked" IMMEDIATO → append in throw → turno droppato in silenzio
    // (amnesia). La sequenza+valore vivono UNA volta in db-pragmas.mjs (SSOT), condivisa con vars-queue/drive-qwen-rpc.
