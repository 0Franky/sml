...
TIME: in the CHRONOLOGICAL lanes — <messages_with_user> and <last_tool_calls> — each line carries a [+Xs] shift = seconds since session start (the absolute session_start is in the lane header). There the AUTHORITATIVE order is given by these timestamps, NOT by the position of the lines — do not assume the lines are pre-sorted; if a shift is out of order, trust the shift. Reconstruct the real timeline from the [+Xs] values. (The other lanes are ordered by state, not time: <facts> by importance, <task_list> by priority/status, <vars> by name — do not read a timeline into their order.)
...
Your earlier turns are NOT in the chat array — they are kept in the <context> lanes below, which ARE your memory: <messages_with_user> = the dialogue so far (the user's messages and your replies), <last_tool_calls> = your past actions, <task_list>/<current_aim>/<vars> = your working state. A lane that isn't shown is empty. TIME: in <messages_with_user> and <last_tool_calls> each line has a [+Xs] shift (seconds since session start, header has the absolute start) — there the shifts, not the line order, are the authoritative timeline; the other lanes (<facts> by importance, <task_list> by state, <vars> by name) are NOT time-ordered. Before something scrolls out of the rolling <messages_with_user> window, save it self-contained: note("<a full sentence>", key) for a durable fact, set_var for a structured value, jot("<a full sentence>") for a VOLATILE working note (appears in <scratch>, rolling — recent shown, old fade). Answer ONLY from what the lanes actually contain; if it isn't there, say so — never invent it.
</how_memory_works>
`;
}
